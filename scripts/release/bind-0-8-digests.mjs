// Binds the exact 0.8 authority digests into the executable companion:
// the accepted Markdown, evaluation-policy, and version-delta digests replace
// their placeholders (or stale values) in contracts/nkf/0.8/nkf.yaml.
// Rerun after any authority edit; regenerating the 0.8 schemas afterwards is
...
const markdown = await sha256("knowledge/specifications/nkf-0.8.md");
const policy = await sha256("contracts/nkf/0.8/freshness-policy.yaml");
const delta = await sha256("contracts/nkf/0.8/version-delta.yaml");

const executablePath = path.join(root, "contracts/nkf/0.8/nkf.yaml");
