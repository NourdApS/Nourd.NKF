...
export async function buildPublicDocs(root = repositoryRoot) {
  ...
  await copyFile(
    ...
    path.join(docsRoot, "reference/nkf-0.4.md"),
  );
  await copyFile(
    path.join(root, "knowledge/specifications/nkf-0.5-revision-2.md"),
    path.join(docsRoot, "reference/nkf-0.5.md"),
  );
  await copyFile(
    path.join(root, "knowledge/specifications/nkf-0.6-revision-3.md"),
    path.join(docsRoot, "reference/nkf-0.6.md"),
  );
  await copyFile(
  ...
  for (const directory of [".agents", ".claude"]) {
    await copyFile(
      path.join(root, `distribution/nkf/0.6/${directory}/skills/nkf-onboarding/SKILL.md`),
      path.join(docsRoot, `${directory}/skills/nkf-onboarding/SKILL.md`),
    );
  }
  for (const kind of ["product", "technology"]) {
    const fixture = kind === "product" ? "minimal-0-6" : "technology-0-6";
