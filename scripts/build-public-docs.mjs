...
export async function buildPublicDocs(root = repositoryRoot) {
  ...
  await copyFile(
    path.join(root, "distribution/nkf/0.5/integrations/onboarding/nkf-onboarding-protocol.md"),
    path.join(docsRoot, "tools/nkf-onboarding-protocol.md"),
  );
  for (const directory of [".agents", ".claude"]) {
    await copyFile(
      path.join(root, `distribution/nkf/0.5/${directory}/skills/nkf-onboarding/SKILL.md`),
      path.join(docsRoot, `${directory}/skills/nkf-onboarding/SKILL.md`),
    );
  }
  for (const kind of ["product", "technology"]) {
    const fixture = kind === "product" ? "minimal-0-5" : "technology-0-5";
