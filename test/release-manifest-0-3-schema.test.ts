...
const schema = JSON.parse(
  await readFile(
    path.join(repositoryRoot, "contracts/nkf/0.3/schemas/release-manifest.schema.json"),
    "utf8",
  ),
);
...
function manifest() {
  return {
    ...
    authority: {
      ...
      markdown: { path: "knowledge/specifications/nkf-0.3.md", digest: digest() },
      executable: { path: "contracts/nkf/0.3/nkf.yaml", digest: digest() },
    },
    schemas: [
      ...
      identity: `urn:nkf:0.3:schema:${identity}`,
      path: `contracts/nkf/0.3/schemas/${file}`,
      digest: digest(),
    })),
    files: [
      {
...
describe("NKF 0.3 release-manifest Schema", () => {
