...
// The 0.81 release set is generated by the later release task, so the 0.81
// manifest construction is exercised end to end by the adopter suite's
// substituted candidate archive; here the predecessor 0.8 manifest verifies
// against genuine release-set bytes and the current 0.81 schema verifies its
// strict compile and exact authority binding.
const schemaPath = path.join(repositoryRoot, "contracts/nkf/0.8/schemas/release-manifest.schema.json");
const currentSchemaPath = path.join(repositoryRoot, "contracts/nkf/0.81/schemas/release-manifest.schema.json");
...
describe("NKF release-manifest schemas", () => {
  it("compiles strictly and accepts the exact constructed 0.8 manifest", async () => {
  ...
  it("compiles the current 0.81 schema strictly with the exact authority binding", async () => {
    ...
    expect(schema.$id).toBe("urn:nkf:0.81:schema:release-manifest");
    ...
    expect(relabeled.files.some((file: { path: string }) =>
      file.path === "contracts/nkf/0.81/version-delta.yaml")).toBe(false);
  });
