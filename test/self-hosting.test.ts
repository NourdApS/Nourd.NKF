...
describe("NKF repository self-hosting", () => {
  it("conforms as the live 0.8 producer and as the promoted 0.81 producer", async () => {
    ...
    expect(recordIds).toContain("nkf-0.8-specification");
    ...
    const carriesCandidateEvidence = (bundle.non_records ?? []).some(
      (entry: { document?: { id?: string } }) =>
        entry.document?.id === "document-nkf-0-81-specification",
    );

    if (result.nkf_version === "0.81") {
      // The promoted producer: native accepted 0.81 Specification and no
      // remaining candidate Evidence representation.
      expect(recordIds).toContain("nkf-0.81-specification");
    ...
    // The pre-promotion producer: the live bundle still declares exactly 0.8,
    // carries the 0.81 Specification as candidate Evidence, and has no native
    // 0.81 Specification record yet.
    expect(result.nkf_version).toBe("0.8");
    expect(recordIds).not.toContain("nkf-0.81-specification");
