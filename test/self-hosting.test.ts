...
describe("NKF repository self-hosting", () => {
  it("conforms as the live 0.71 producer and as the promoted 0.8 producer", async () => {
    ...
    const carriesCandidateEvidence = (bundle.non_records ?? []).some(
      (entry: { document?: { id?: string } }) =>
        entry.document?.id === "document-nkf-0-8-specification",
    );

    if (result.nkf_version === "0.8") {
      // The promoted producer: native accepted 0.8 Specification and no
    ...
    // The pre-promotion producer: the live bundle still declares exactly 0.71,
    // carries the 0.8 Specification as candidate Evidence, and has no native
    // 0.8 Specification record yet.
    expect(result.nkf_version).toBe("0.71");
    expect(recordIds).not.toContain("nkf-0.8-specification");
