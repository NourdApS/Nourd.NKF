...
async function copyAuthority(): Promise<string> {
  ...
  await cp(
    ...
    path.join(root, "contracts/nkf/0.8"),
    { recursive: true },
  );
  return path.join(root, "contracts/nkf/0.8");
}

describe("current NKF 0.8 contract realization", () => {
  ...
  it("strictly loads the registered 0.71 predecessor authority and schema bindings", async () => {
    const loaded = await loadContracts(
      path.join(repositoryRoot, "contracts/nkf/0.71"),
      VERSION_BINDINGS["0.71"],
      "0.71",
    );
    expect(loaded.diagnostics).toEqual([]);
    expect(loaded.executable.contract).toBe("nkf.contract-set");
  ...
  it("distinguishes unavailable and mismatched authority pairs", async () => {
    ...
    await unlink(
      path.resolve(unavailableRoot, "../../../knowledge/specifications/nkf-0.8.md"),
    );
    expect(
      (await loadContracts(unavailableRoot, VERSION_BINDINGS["0.8"], "0.8")).diagnostics.map(
    ...
    await writeFile(
      path.resolve(mismatchedRoot, "../../../knowledge/specifications/nkf-0.8.md"),
      "# Changed\n",
      "utf8",
    );
    expect(
      (await loadContracts(mismatchedRoot, VERSION_BINDINGS["0.8"], "0.8")).diagnostics.map(
        (diagnostic) => diagnostic.rule_id,
      ),
