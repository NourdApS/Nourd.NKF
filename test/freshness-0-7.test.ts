...
const contractRoot = path.join(repositoryRoot, "contracts/nkf/0.7");
...
const fixture = path.join(repositoryRoot, "fixtures/valid/minimal-0-7");
const technologyFixture = path.join(repositoryRoot, "fixtures/valid/technology-0-7");
...
async function copyFixture(source: string): Promise<string> {
  const project = await mkdtemp(path.join(tmpdir(), "nkf-0-7-"));
...
describe("NKF 0.7 digest-bound freshness", () => {
