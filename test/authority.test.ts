...
describe("canonical NKF 0.2 authority realization", () => {
  it("keeps the corrected 0.2 authority pair bound to ADR 0104", async () => {
    const [specification, executable, acceptanceDecision] =
      await Promise.all([
        ...
        readFile(
          path.join(
            repositoryRoot,
            "knowledge/decisions/0104-accept-the-cancelled-state-pair.md",
          ),
          "utf8",
        ),
      ]);
  ...
  it("keeps strict executable parsing, Markdown binding, and 159-rule severity parity", async () => {
    ...
    expect(executableRules.size).toBe(159);
