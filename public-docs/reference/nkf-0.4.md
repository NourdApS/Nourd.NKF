---
id: nkf-0.4-specification
title: NKF 0.4 — Product And Technology Knowledge Format
type: specification
summary: NKF 0.4 preserves the complete NKF 0.3 Product and Technology contracts while shipping a separately frozen, non-breaking dependency-security maintenance set.
created_at: 2026-08-11T14:05:08Z
...
# NKF 0.4 — Product And Technology Knowledge Format

- **Predecessor canonical baseline:** `knowledge/specifications/nkf-0.3.md`
- **Predecessor canonical digest:** `0094bedce5485901c3ab6fb542e3d2785e961991cf7cc6f24e9aa3462498436e`
- **Predecessor executable baseline:** `contracts/nkf/0.3/nkf.yaml`
- **Predecessor executable digest:** `e988a596e741d48611a5f77a9236e9d539f9a7475a76f07768bc273a8bf4d27f`
- **Canonical destination:** `knowledge/specifications/nkf-0.4.md`
- **Executable companion destination:** `contracts/nkf/0.4/nkf.yaml`
- **Governing successor direction:** [ADR 0112](../decisions/0112-allocate-nkf-0-4-security-maintenance.md)
- **Independent governing inputs:** the accepted NKF 0.3 authority pair through
  [ADR 0110](../decisions/0110-accept-the-nkf-0-3-authority-pair.md), with
  immutable publication and self-adoption boundaries from
...
> This exact revision is the canonical NKF 0.4 specification, accepted together
> with its digest-bound executable companion by the technical reviewer under
> [ADR 0112](../decisions/0112-allocate-nkf-0-4-security-maintenance.md)'s
> explicit maintenance delegation. It governs repositories that deliberately
> declare NKF 0.4 and does not alter the immutable authority of NKF 0.1,
> NKF 0.2, or NKF 0.3.

## Purpose

The **Nourd Knowledge Format (NKF)** is a human-readable, machine-verifiable
format for durable governed knowledge. NKF 0.4 supports Product and Technology
...
NKF 0.4 is a pre-stable format version produced under the versioned contract
evolution accepted by [ADR 0076](../decisions/0076-versioned-contract-evolution.md).
The complete accepted NKF 0.3 authority pair and lineage remain the predecessor
meaning, including the Product and Technology profiles, Decision Applicability
Gate, deep-link and title rules, and four-state Task vocabulary.
[ADR 0112](../decisions/0112-allocate-nkf-0-4-security-maintenance.md)
allocates the non-breaking dependency-security successor to NKF 0.4 while
preserving the format meaning accepted for 0.3. Publication still freezes the
exact complete set, a published release is never replaced in place, and exact
candidate self-adoption precedes publication. This revision governs a
repository only after it is accepted and that repository deliberately
declares `nkf_version` `0.4`; NKF 0.1, NKF 0.2, and NKF 0.3 remain immutable
...
[ADR 0109](../decisions/0109-publication-freeze-and-proven-self-adoption.md).
This NKF 0.4 revision preserves that complete predecessor meaning and changes
only the format coordinate, explicit predecessor compatibility, patched
dependency closure, and exact complete-set bindings authorized by
[ADR 0112](../decisions/0112-allocate-nkf-0-4-security-maintenance.md). NKF
0.1, NKF 0.2, and NKF 0.3 remain immutable authority for repositories that
declare them, and consumers migrate deliberately.

## Scope

NKF 0.4 defines Common contracts and two concrete Root Profiles: Product and
...
to NKF 0.4 is `breaking` and requires repository-owner approval before
mutation, even though migration is designed to preserve existing knowledge.
Adoption from NKF 0.3 to NKF 0.4 is `non-breaking`, requires no knowledge
migration, and requires no repository-owner breaking approval because 0.4
preserves the accepted 0.3 format meaning. An exact NKF 0.4-to-0.4 refresh is
also `non-breaking`, requires no migration, and requires no repository-owner
approval because the governing format version does not change.

NKF has one version namespace: the NKF format version. A bundle MUST declare
`nkf_version`. NKF 0.4 uses the unversioned canonical identities `nkf.bundle`,
...
identities. They are all governed by the one NKF version coordinate, `0.4`.
Profile identity is not an independent version coordinate.

The sole NKF 0.4 record definition includes explicit responsibility bindings.
...
migrated by their consumers, but MUST NOT be reported as native NKF 0.4
...
version `0.4` alone does not identify editorial changes to a draft.

NKF 0.4 uses OKF 0.2 as its interoperability baseline. A later OKF release
...
contracts. NKF 0.4 does not silently rename or reinterpret them. A controlled
...
that directly contains `.nourd/`. Native NKF 0.4 fixes:
...
| --- | --- |
| `nkf_version` | Constant NKF version `"0.4"` |
...
top-level fields fail closed. Native NKF 0.4 has no `product_record`,
...
Every Product and Technology bundle using this successor NKF 0.4 revision
...
Native NKF 0.4 interprets the Markdown source body using CommonMark 0.31.2.
...
above, NKF 0.4 does not impose a narrower lexical format on the optional
...
Native NKF 0.4 defines no presentation-guidance field. Markdown is the default
...
Cross-bundle identity and typed relationships are deferred in NKF 0.4.
...
NKF 0.4 defines these record relationship types:
...
states the claimed original acceptance date. Native NKF 0.4 has no universal
...
last-modified times. NKF 0.4 does not define `stale_after` or a universal
freshness policy.
...
`nkf_version: "0.4"` is the only version coordinate and artifact digests bind
exact revisions.
...
2. executable YAML with identity `nkf.extension`, `nkf_version: "0.4"`, the
...
Native NKF 0.4 accepts no concrete extension. Portable presentation guidance
...
The fixed `.nourd` layout in the bundle contract is native NKF 0.4, not a
...
NKF 0.4 is based on the official
...
```text
contracts/nkf/0.4/schemas/
  bundle.schema.json   # urn:nkf:0.4:schema:bundle
  record.schema.json   # urn:nkf:0.4:schema:record
  validation-result.schema.json
                       # urn:nkf:0.4:schema:validation-result
```

The `0.4` component is the one NKF version coordinate. Each schema carries
...
contracts/nkf/0.4/schemas/release-manifest.schema.json
                       # urn:nkf:0.4:schema:release-manifest
```
...
closed UTF-8 JSON `nkf.validation-result` object with `nkf_version: "0.4"`.
...
`nkf.validation-snapshot` JSON with `nkf_version: "0.4"` and one entry per
...
snapshot, its core artifact bindings match the current accepted NKF 0.4
...
process. NKF 0.4 does not infer a hidden acceptance change, poll an authority,
...
NKF 0.4 uses one content-addressed archive attached to a Github Release in
...
They are not NKF versions. `nkf_version: "0.4"` remains the only format
version.

### Release Manifest

The archive contains exactly one UTF-8 JSON `release-manifest.json` and one
UTF-8 YAML `contracts/nkf/0.4/release-set.yaml`. The release manifest has
contract identity `nkf.release-manifest` and `nkf_version: "0.4"`; the release
set has contract identity `nkf.release-set` and the same version.

The manifest is one closed JSON object with exactly seven required top-level
...
- `knowledge/specifications/nkf-0.4.md`; and
- `contracts/nkf/0.4/nkf.yaml`.

`schemas` contains exactly four entries in exact identity order:

1. `urn:nkf:0.4:schema:bundle`;
2. `urn:nkf:0.4:schema:record`;
3. `urn:nkf:0.4:schema:release-manifest`; and
4. `urn:nkf:0.4:schema:validation-result`.
...
The manifest's checker, authority, and schemas bindings repeat identities for
...
`contracts/nkf/0.4/release-set.yaml` is the sole exact coverage, membership,
class, and mode enumeration for the NKF 0.4 complete set. It is a closed YAML
...
members and modes declared by `contracts/nkf/0.4/release-set.yaml` under stable
...
0.4 is pre-stable. Before publication, release automation downloads the
...
control does not alter the NKF 0.4 format.
...
but NKF 0.4 does not define that policy. A consumer MUST NOT hide missing
...
The following remain deliberately unresolved in NKF 0.4:
...
profile decision is part of NKF 0.4 core.

## Minimal Example

A logical NKF 0.4 manifest:
...
| --- | --- | --- |
| NKF attempts to model every kind of knowledge immediately | The first version becomes unusable and untestable | Keep 0.4 limited to evidence-backed Product and Technology profiles and add later roots from real needs |
