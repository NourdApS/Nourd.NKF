...
title: NKF 0.6 — Product And Technology Knowledge Format
type: specification
summary: NKF 0.6 corrects native governed mutation, requires complete ordinary producer authoring proof before publication, carries Apache-2.0 and third-party notices in the complete release set, and updates current distribution identity while preserving NKF 0.5 graph and freshness meaning.
created_at: 2026-08-14T00:16:32Z
---

# NKF 0.6 — Product And Technology Knowledge Format

- **Predecessor canonical baseline:** `knowledge/specifications/nkf-0.5-revision-2.md`
- **Predecessor canonical digest:** `0f3b7c085eba4fa92655e20916fccb7013169b5560dd50c241fb4726df31287c`
- **Predecessor executable baseline:** `contracts/nkf/0.5/revision-2/nkf.yaml`
- **Predecessor executable digest:** `2743102a4bddf9a26253fba3982f00bf9c688c819891815221e3ea4ee67c5290`
- **Predecessor evaluation policy:** `contracts/nkf/0.5/freshness-policy.yaml`
- **Predecessor evaluation-policy digest:** `5789b935e8df4fa39462846481f3302d072c722fa106da5d136952cb9c993cdd`
- **Canonical destination:** `knowledge/specifications/nkf-0.6-revision-3.md`
- **Executable companion destination:** `contracts/nkf/0.6/revision-3/nkf.yaml`
- **Evaluation-policy destination:** `contracts/nkf/0.6/freshness-policy.yaml`
- **Evaluation-policy digest:** `a870dc037364776b927e93705655d3f4572407c65852c4e5a258aa7d5bafe4a4`
- **Governing successor direction:** [ADR 0121](../decisions/0121-adopt-the-nkf-0-6-corrective-and-licensing-direction.md)
- **Independent governing inputs:** the exact NKF 0.5 authority set accepted by
  [ADR 0119](../decisions/0119-accept-the-nkf-0-5-revision-2-authority-pair.md),
  the corrective and licensing direction adopted by
  [ADR 0121](../decisions/0121-adopt-the-nkf-0-6-corrective-and-licensing-direction.md),
  the Human Product Owner's later explicit confirmation of the exact,
  non-reusable two-occurrence historical containment for
  [ADR 0122](../decisions/0122-accept-the-nkf-0-6-authority-set.md), to be
  recorded by the future correcting acceptance Decision,
...
> This exact revision is the canonical NKF 0.6 specification when accepted
> together with its digest-bound executable companion by the technical
> reviewer under the Human Product Owner's explicit derivation delegation.
> It realizes only the predecessor meaning, the direction adopted by
> [ADR 0121](../decisions/0121-adopt-the-nkf-0-6-corrective-and-licensing-direction.md),
> and the later Human-confirmed exact two-occurrence
> [ADR 0122](../decisions/0122-accept-the-nkf-0-6-authority-set.md) containment
> correction that the future correcting acceptance Decision must bind without
> creating a generalized bypass;
> governs repositories that deliberately declare NKF 0.6, and does not alter
> the immutable authority of NKF 0.1 through NKF 0.5.

This revision 3 is a prepublication structural successor to the exact
revision 2 authority accepted by
[ADR 0124](../decisions/0124-accept-the-nkf-0-6-revision-2-authority-set.md).
Revision 2 named its future accepting Decision as an unlinked record-ID token;
that token became a resolvable same-bundle reference only when
[ADR 0124](../decisions/0124-accept-the-nkf-0-6-revision-2-authority-set.md)
was created, making the accepted source structurally nonconformant. Revision 3
changes only that acceptance-time reference and the exact successor bindings
required to select these bytes. It adds no Product meaning, keeps revision 2
immutable governed history, and requires the correction Decision to exist and
resolve during the exact authority audit rather than after it.

The published NKF 0.5 complete set remains immutable at its exact archive,
authority, checker, adopter, and repository coordinates. Ordinary producer
authoring exposed a layout-sensitive native record-repin defect only after
publication. This successor corrects that defect, makes mutation results
truthful, moves complete ordinary producer authoring proof before publication,
adds the confirmed Apache-2.0 licensing set, and updates current distribution
identity. It adds no deferred Product capability and does not rewrite any
published 0.5 member.

## Purpose

The **Nourd Knowledge Format (NKF)** is a human-readable, machine-verifiable
format for durable governed knowledge. NKF 0.6 supports Product and Technology
...
NKF 0.6 is a pre-stable format version produced under the versioned contract
evolution accepted by [ADR 0076](../decisions/0076-versioned-contract-evolution.md).
The complete accepted NKF 0.5 authority set and lineage remain the predecessor
meaning, including both Root Profiles, stable document nodes, declared
lifecycle, the deterministic graph, contextual freshness, conservative impact
review, revision-bound readiness, the Decision Applicability Gate, deep-link
and title rules, four-state Task vocabulary, immutable release set, and public
Adopt workflow. [ADR 0121](../decisions/0121-adopt-the-nkf-0-6-corrective-and-licensing-direction.md)
adopts only the corrective authoring, stronger prepublication proof,
Apache-2.0 licensing-preparation, and current repository-identity direction.
...
authoring rehearsal, and audit precede publication; ordinary public producer
...
`nkf_version` `0.6`; earlier NKF versions remain immutable authority for
...
declares NKF 0.5, as required by the prepublication sequence. Its frontmatter
is therefore the exact stable native 0.5 source envelope. Before candidate
Adopt it is represented as non-authoritative candidate Evidence at the exact
future canonical source path rather than a 0.5 Specification record, and the
candidate executable and policy are carried only as non-authoritative
prospective artifacts. A later accepted Decision may bind these exact
Markdown, executable-companion, and policy bytes plus one exact producer-only
promotion input at
`knowledge/evidence/release/nkf-0.6-revision-3-producer-promotion.yaml`. That input is a
strict non-authoritative Evidence YAML artifact; it contains the exact three
authority bindings and the complete future native Specification record
declaration plus the exact destination
`.nourd/knowledge/records/nkf-0.6-specification-revision-3.yaml`, including the prospective
`immutable`/`accepted` governance state
and accepting-authority identifier that candidate Adopt may apply only after
the separate acceptance Decision resolves. It contains no accepting-Decision
digest and neither its declared future state nor its validation proves or
performs acceptance. The accepting Decision separately binds the exact
promotion-input digest, avoiding a self-reference cycle.

[ADR 0124](../decisions/0124-accept-the-nkf-0-6-revision-2-authority-set.md)
is the immutable accepted predecessor release-authority selection for NKF 0.6.
Its exact source SHA-256 is
`7a5310ccc473e6e29f46a206741e44582c0226c7f32c8eafae385e3b0121f048`
and its exact native declaration SHA-256 is
`2a135504e994d94f69ca09bd42e7aa80c5e77d2cb7fddb90790c2bb8bc31e652`.
Both remain unchanged, governed, and graph-visible accepted history. The later
correction Decision MUST explicitly supersede only that Decision's selection of the
revision 2 NKF 0.6 Specification, executable, policy, and producer-promotion
input as current release authority, and MUST select this revision-3 set
instead. It does not retract that predecessor Decision, alter its historical acceptance fact,
or supersede its other recorded conclusions. Until that exact correction
Decision exists, this revision remains prospective and cannot govern a release.

At both authorized producer stages, 0.6 Adopt verifies the exact accepted
three-file set, the exact accepted promotion input, and the accepting Decision,
preserves the Markdown byte for byte, removes exactly that candidate Evidence
document representation, performs the exact historical-containment transition
declared below, and creates the native 0.6 Specification declaration
at the exact supplied declaration destination, lifecycle `immutable`, status
`accepted`, bound to the exact accepting
Decision, source digest,
[NKF-027](../tasks/items/NKF-027-correct-and-release-nkf-0-6-with-open-source-licensing.md),
and the complete exact section and
relationship declaration supplied by the accepted promotion input. A separate
reviewer-completed `nkf.semantic-review-input` for the exact post-promotion
whole-root candidate is required at execution time and produces the native
0.6 confirmed baseline; its review facts are not authority-pair meaning and
are not inferred by the tool. No ordinary consumer update can invoke this
producer-only promotion. Validation, migration, declaration generation, or
the promotion input cannot supply acceptance or derive the semantic section
map.
...
Adopt returns `current` only after it re-verifies the exact native 0.6
declaration, absence of the candidate Evidence and original acceptance
Decision's native declaration, the one exact locked historical Evidence
document and accepting-Decision binding,
generated integration, candidate graph, and confirmed baseline. It performs no
promotion mutation again. Any non-producer repository, missing or extra
candidate Evidence representation, pre-existing conflicting declaration, or
stage outside those two exact producer cases fails before mutation.

The original NKF 0.6 acceptance Decision at
`knowledge/decisions/0122-accept-the-nkf-0-6-authority-set.md` remains immutable
accepted history, but its exact source contains exactly two structurally
invalid plain same-bundle references: the Task
[NKF-027](../tasks/items/NKF-027-correct-and-release-nkf-0-6-with-open-source-licensing.md)
in `Scope And Applicability`, and
[ADR 0121](../decisions/0121-adopt-the-nkf-0-6-corrective-and-licensing-direction.md)
in `Alternatives Considered`. The producer
promotion MUST remove only its exact native record declaration at
`.nourd/knowledge/records/adr-0122.yaml` and represent the unchanged source as
an Evidence document whose identity, stable path, source digest, predecessor
declaration digest and both diagnostic occurrences are all fixed by
the accepted promotion input. That document MUST carry the closed
`historical_acceptance_attempt_lock` defined by this revision. It remains
digest-bound, graph-visible accepted history and is not current release
authority.

The lock contains exactly `record_id`, `record_declaration`, `source`,
`violations`, `historical_effect`, and `correction_decision`. The record
declaration and source bindings each contain
exact project-relative `path` plus SHA-256 `digest`. The only permitted values
are the exact [ADR 0122](../decisions/0122-accept-the-nkf-0-6-authority-set.md)
bindings in the accepted producer-promotion input. `violations` is an
order-semantic array containing exactly two closed entries, each with exactly
`diagnostic`, `section`, `token`, and `occurrence`. Both diagnostics are
`markdown.reference.deep-link.required`: the first binds section
`scope-and-applicability`, token
[NKF-027](../tasks/items/NKF-027-correct-and-release-nkf-0-6-with-open-source-licensing.md),
occurrence `1`; the second binds section `alternatives-considered`, token
[ADR 0121](../decisions/0121-adopt-the-nkf-0-6-corrective-and-licensing-direction.md),
occurrence `1`. The
historical effect is
`accepted-history-not-current-release-authority`. The correction Decision ID is
[`adr-0125`](../decisions/0125-accept-the-nkf-0-6-revision-3-authority-set.md);
its digest is supplied and verified separately as the exact
accepting Decision at promotion time, avoiding a self-reference cycle.

The checker still parses and digest-binds the complete unchanged
[ADR 0122](../decisions/0122-accept-the-nkf-0-6-authority-set.md) source,
includes the Evidence node in graph and freshness evaluation, and
counts both contained occurrences. It suppresses only those two diagnostics when
every lock field, byte digest, declaration digest, section, token, occurrence,
Evidence identity, and accepting-Decision binding is exact. A missing, extra,
changed, duplicated, or relocated occurrence; any changed source or
declaration byte; reuse of the lock on another node; or any other deep-link
violation fails closed. The containment neither rewrites nor retracts
[ADR 0122](../decisions/0122-accept-the-nkf-0-6-authority-set.md), does not make
it current authority, and is not a reusable Evidence exemption.
...
declaration exceptions after update. Their exact source bytes, identities,
locks, and 0.5 meaning remain immutable. The exceptions remain restricted to
the two exact 0.5 Specification identities and are unavailable to new
0.6-native records.
...
[ADR 0109](../decisions/0109-publication-freeze-and-proven-self-adoption.md).
NKF 0.4 then shipped the separately frozen dependency-security maintenance
set through [ADR 0113](../decisions/0113-accept-the-nkf-0-4-authority-pair.md).
NKF 0.5 then shipped stable document nodes, stable source paths across
lifecycle changes, one declared graph, contextual freshness, conservative
impact closure, baseline confirmation, and revision-bound receipts and
readiness through [ADR 0119](../decisions/0119-accept-the-nkf-0-5-revision-2-authority-pair.md).
This NKF 0.6 revision preserves that complete predecessor meaning and adds
only the exact corrective, prepublication-proof, licensing, and current-
identity direction adopted by
[ADR 0121](../decisions/0121-adopt-the-nkf-0-6-corrective-and-licensing-direction.md),
plus the later Human-confirmed exact, non-reusable containment of the two
identified [ADR 0122](../decisions/0122-accept-the-nkf-0-6-authority-set.md)
deep-link diagnostics that the future correcting
acceptance Decision must accept.
Earlier versions remain immutable authority for repositories that declare
them, and consumers update deliberately.

## Scope

NKF 0.6 defines Common contracts and two concrete Root Profiles: Product and
...
format-visible predecessor compatibility: migration from NKF 0.1, NKF 0.2,
NKF 0.3, or NKF 0.4 to NKF 0.6 is `breaking` and requires repository-owner
approval before mutation, even though migration preserves canonical Markdown
bytes and historical authority except for the already-governed retrospective
Task-gate normalization defined below. The break allocates stable lifecycle paths,
document-node identities, a declared graph, and a reviewed graph baseline.
Updating an exact conformant NKF 0.5 repository to NKF 0.6 is `non-breaking`,
requires no repository-owner approval, preserves canonical Markdown and graph
meaning, preserves exact predecessor-only declaration locks, updates
versioned declarations and operational tooling, and requires no
new semantic baseline when the exact reviewed graph meaning remains
unchanged. The 0.6 policy differs from the 0.5 policy only in its version and
identity coordinates. A 0.5 confirmed baseline may therefore be carried
forward mechanically only when the exact policy objects are identical after
removing top-level `nkf_version` and `id`, every node revision, authored edge,
external dependency, authority input, applicability classification, Decision
classification, relationship-coverage entry, observation, limitation, and
confirmation is unchanged, and the predecessor baseline was current for that
exact 0.5 graph. The updater changes only the baseline contract version,
policy identity and digest, and derived graph revision through structural
scalar mutation; it preserves every reviewed classification and confirmation
byte and records the exact before/after revisions in the Adopt operation
result. If any precondition differs, automatic 0.5-to-0.6 update fails before
mutation rather than inventing or silently discarding semantic review.
A repository whose 0.5 baseline is missing, outdated, disputed, ambiguous, or
otherwise not ready is therefore ineligible for automatic carry-forward. Any
review and sealing needed to restore a current 0.5 predecessor baseline is a
separate governed 0.5 knowledge-maintenance operation: the 0.6 updater MUST
NOT perform it, infer its semantic content, or treat it as part of the update.
An exact NKF 0.6-to-0.6 refresh is `non-breaking`, requires no migration, and
...
`nkf_version`. NKF 0.6 uses the unversioned canonical identities `nkf.bundle`,
...
identities. They are all governed by the one NKF version coordinate, `0.6`.
Profile identity is not an independent version coordinate.

The sole NKF 0.6 record definition includes explicit responsibility bindings.
...
migrated by their consumers, but MUST NOT be reported as native NKF 0.6
...
version `0.6` alone does not identify editorial changes to a draft.

NKF 0.6 uses OKF 0.2 as its interoperability baseline. A later OKF release
...
contracts. NKF 0.6 does not silently rename or reinterpret them. A controlled
...
that directly contains `.nourd/`. Native NKF 0.6 fixes:
...
| --- | --- |
| `nkf_version` | Constant NKF version `"0.6"` |
...
top-level fields fail closed. Native NKF 0.6 has no `product_record`,
...
and `redirect`, and optional for `navigation` and `other`. When present it is
...
becomes its document-node ID during migration. A new 0.6 Task is created with
...
```yaml
knowledge_graph:
  policy: nkf.freshness-policy.0.6
...
NKF 0.6, outside canonical Markdown, and governed by the baseline contract
...
native 0.6 checker matches a supplied external observation by that exact ID
...
Every Product and Technology bundle using this successor NKF 0.6 revision
...
Every Task and Design source path that exists at 0.6 migration is frozen as
that node's stable locator, including a legacy path containing an earlier
lifecycle segment. Migration MUST NOT move that source or rewrite inbound
links merely to reconcile lifecycle. New Task and Design sources use neutral
`tasks/items/` and `designs/items/` paths. A lifecycle transition changes only
the declaration state and generated navigation; it MUST NOT move or rewrite
the canonical Markdown source. Unresolved pre-NKF Markdown may remain at a
non-canonical path while represented appropriately. Once a project adopts
this successor revision, the complete topology, stable-path rule, and
generated navigation agreement are continuing conformance requirements.
...
Breaking migration from NKF 0.1, NKF 0.2, NKF 0.3, or NKF 0.4 preserves every
canonical Markdown byte except the one closed retrospective Task-gate
normalization below, and
preserves every existing Task and Design source path. It upgrades declarations,
allocates and persists document-node IDs, converts lifecycle truth from source
frontmatter and path placement into declaration state, generates the canonical
graph candidate, and regenerates navigation. Every preserved predecessor
source that contains a now-forbidden mutable-state key is classified as
`legacy-locked` and remains byte-exact; its historical state fields are not
current authority. A preserved source already using the native envelope needs
no lock. New or deliberately
rewritten 0.6 sources use the native 0.6 envelope.

Before the 0.6 declaration, source digest, legacy lock, graph revision, or
baseline is derived, every predecessor Task that lacks the Decision
Applicability Gate MUST receive one truthful retrospective gate under
[ADR 0077](../decisions/0077-decision-applicability-gate.md). The exact gate
Markdown MUST be supplied by the named semantic reviewer through the Adopt
review input. That closed review document has two deterministic stages. When
at least one predecessor Task lacks the gate, an initial `predecessor-gates`
stage carries one `retrospective_gate_review` object with reviewer identity,
review instant, and a non-empty `gates` array. Each item binds the exact Task
ID, original predecessor source SHA-256, and complete UTF-8 LF gate block. The
object's reviewer identity and review instant apply to every item. The array
MUST cover each and only gate-free
predecessor Task exactly once,
MUST pass the native gate grammar and completion rule, and MUST end with the
following exact two-line paragraph, replacing `<predecessor>` with the
declared predecessor version:

```text
This gate was added retrospectively during the NKF <predecessor>-to-0.6 migration; no
historical extraction is implied.
```

A deterministic tool MAY append that exact
reviewed block after the fixed UTF-8 suffix `\n\n`; the gate block begins with
the exact line `## Decision Applicability` and ends with one LF. It MUST NOT select applicable
Decisions, capabilities, findings, verification levels, or exceptions. Missing,
placeholder, ambiguous, extra, or nonconformant gate review fails before
project mutation. Existing gate bytes are never replaced. Every non-gate
canonical Markdown byte remains exact, and the post-normalization source bytes
become the sole source bytes bound by the 0.6 document digest and any
`legacy_lock`.

When the gate stage is complete, Adopt verifies and applies it only in the
isolated candidate, derives the exact 0.6 graph, and replaces the external
review template with a `whole-root` stage that preserves the exact complete
`retrospective_gate_review`
but resets all graph-review fields to explicit placeholders. Adopt again stops
before project mutation. Only after the named reviewer completes the exact
node, relationship, Decision, observation, limitation, claim, and dispute
fields may the baseline be sealed and the candidate applied. If no predecessor
Task lacks a gate, the first generated template is already `whole-root` and
forbids `retrospective_gate_review`. One public Adopt command therefore remains
the only operation; gate-free predecessors require two review stops rather
than allowing the tool to invent either historical gate meaning or graph
completeness.

Mechanical migration cannot confirm semantic-edge completeness. Before the
candidate may be sealed or applied, a named human or agent performs a
whole-root semantic review, supplies exact review evidence, and confirms the
candidate graph baseline. Missing, outdated, disputed, or ambiguous baseline
confirmation produces `unknown` and blocks adoption. The repository-owner
breaking approval and the semantic reviewer may be different actors; neither
act implies acceptance of canonical Product or Technology meaning.

Ordinary update from an exact conformant NKF 0.5 repository does not run that
breaking legacy migration or retrospective gate normalization. It uses only
the closed non-breaking 0.5-to-0.6 update defined above, preserves the exact
predecessor-only `legacy_lock`, `accepted_bootstrap_lock`, and
`prepublication_supersession_lock` objects without assigning any of them to a
new 0.6 record, and carries the confirmed baseline only when every exact
equivalence precondition holds. The NKF producer's candidate-bound promotion
is a third, producer-only path: it consumes the accepted serialized promotion
input, removes the exact prospective Evidence representation, creates the
native 0.6 Specification declaration at the supplied destination, and seals
the exact post-promotion graph from a separate completed whole-root review.
The 0.5-only `accepted_bootstrap_lock` is forbidden on that new 0.6 record.
...
Native 0.6 frontmatter is deliberately stable orientation. Mutable lifecycle,
...
a winner or rewrite either representation. Native 0.6 frontmatter MUST NOT
...
the exemption and requires the native 0.6 envelope. This narrow rule preserves
...
state key in native 0.6 source emits
...
Native NKF 0.6 interprets the Markdown source body using CommonMark 0.31.2.
...
above, NKF 0.6 does not impose a narrower lexical format on the optional
...
Native NKF 0.6 defines no presentation-guidance field. Markdown is the default
...
Cross-bundle identity and typed relationships are deferred in NKF 0.6.
...
NKF 0.6 defines these record relationship types:
...
```json
{"contract":"nkf.graph-revision","nkf_version":"0.6","bundle":"<bundle-id>","profile":"<profile-id>","nodes":[{"node":<node-reference>,"revision":"<hex>"}],"edges":[<normalized-authored-edge>],"external_dependencies":[<normalized-external-dependency>],"authority_inputs":[<normalized-authority-input>],"policy":{"id":"nkf.freshness-policy.0.6","sha256":"<hex>"}}
```
...
`nkf.freshness-policy.0.6` at
`contracts/nkf/0.6/freshness-policy.yaml`. Its release manifest binds the exact
...
`0.6`. It binds bundle identity, selected profile, graph revision, policy
identity and digest, a sorted complete node-revision map, relationship-category
coverage, purpose-specific applicability coverage and participation roles,
applicable Decision classifications, and one confirmation.

The confirmation contains reviewer kind `human` or `agent`, a non-empty stable
reviewer identity, an exact UTC millisecond review instant, the literal claim
`semantically-reviewed-whole-root`, a non-empty set of exact source-bound
review observations, known limitations, and `disputed: false`. Every graph
relationship category required by the selected profile is marked `reviewed`
or `inapplicable` with a source-bound basis. Empty or generic assertions do not
satisfy review evidence.

A checker validates shape, resolution, exact revisions, coverage, and binding;
it does not establish semantic completeness or reviewer authority. A missing,
outdated, disputed, ambiguous, or unsupported baseline yields `unknown` and
blocks readiness. Recovery is a new whole-root semantic review. Updating the
baseline never rewrites canonical Markdown or the historical predecessor
baseline.

### Operational Receipts

Freshness receipts use contract `nkf.freshness-receipt`, version `0.6`, and are
...
one exact supplied observation identity. NKF 0.6 supports no free-form
...
states the claimed original acceptance date. Native NKF 0.6 has no universal
...
through an explicit 0.6 freshness declaration, policy, purpose, and bound
...
`nkf_version: "0.6"` is the only version coordinate and artifact digests bind
exact revisions.
...
2. executable YAML with identity `nkf.extension`, `nkf_version: "0.6"`, the
...
Native NKF 0.6 accepts no concrete extension. Portable presentation guidance
...
The fixed `.nourd` layout in the bundle contract is native NKF 0.6, not a
...
NKF 0.6 is based on the official
...
```text
contracts/nkf/0.6/schemas/
  bundle.schema.json   # urn:nkf:0.6:schema:bundle
  record.schema.json   # urn:nkf:0.6:schema:record
  validation-result.schema.json
                       # urn:nkf:0.6:schema:validation-result
  graph-baseline.schema.json
                       # urn:nkf:0.6:schema:graph-baseline
  freshness-receipt.schema.json
                       # urn:nkf:0.6:schema:freshness-receipt
  freshness-policy.schema.json
                       # urn:nkf:0.6:schema:freshness-policy
```

The `0.6` component is the one NKF version coordinate. Each schema carries
...
contracts/nkf/0.6/schemas/release-manifest.schema.json
                       # urn:nkf:0.6:schema:release-manifest
```
...
closed UTF-8 JSON `nkf.validation-result` object with `nkf_version: "0.6"`.
...
`nkf.validation-snapshot` JSON with `nkf_version: "0.6"` and one entry per
...
snapshot, its core artifact bindings match the current accepted NKF 0.6
...
process. NKF 0.6 does not infer a hidden acceptance change, poll an authority,
...
| Reviewed baseline seal | One exact reviewer-completed whole-root input and matching candidate graph; it replaces only the graph baseline after exact coverage, basis, Decision reconciliation, observation, limitation, and dispute validation. |
| `Adopt` | Ordinary modes cover the version declaration, release pin, installed checker and adopter, host integration, migrated declarations, graph and baseline inputs, and complete producer or consumer gate. Breaking legacy migration additionally consumes repository-owner approval, any conditionally required retrospective Task-gate review, and the exact completed whole-root review; non-breaking 0.5 update proves every exact baseline-carry-forward precondition. The producer-promotion mode additionally consumes the exact accepted promotion input and accepting Decision, removes the exact candidate Evidence representation, creates the exact supplied native declaration, regenerates navigation, and seals the exact post-promotion graph from the separately completed whole-root review at either authorized producer stage. Rollback covers every project-owned mutation; repeat after promotion returns `current`. |
...
confirmation. A separately authorized publication may publish only the exact
audited candidate bytes. Ordinary producer Adopt after publication remains
...
NKF 0.6 uses one content-addressed archive attached to a Github Release in
...
They are not NKF versions. `nkf_version: "0.6"` remains the only format
version.

### Release Manifest

The archive contains exactly one UTF-8 JSON `release-manifest.json` and one
UTF-8 YAML `contracts/nkf/0.6/release-set.yaml`. The release manifest has
contract identity `nkf.release-manifest` and `nkf_version: "0.6"`; the release
set has contract identity `nkf.release-set` and the same version.

The manifest is one closed JSON object with exactly nine required top-level
fields in this logical shape:

```text
contract, nkf_version, source, checker, authority, freshness_policy, schemas, licensing, files
```
...
- `knowledge/specifications/nkf-0.6-revision-3.md`; and
- `contracts/nkf/0.6/revision-3/nkf.yaml`.

`freshness_policy` binds identity `nkf.freshness-policy.0.6`, exact path
`contracts/nkf/0.6/freshness-policy.yaml`, and its SHA-256 digest.

`schemas` contains exactly seven entries in exact identity order:

1. `urn:nkf:0.6:schema:bundle`;
2. `urn:nkf:0.6:schema:record`;
3. `urn:nkf:0.6:schema:graph-baseline`;
4. `urn:nkf:0.6:schema:freshness-receipt`;
5. `urn:nkf:0.6:schema:freshness-policy`;
6. `urn:nkf:0.6:schema:release-manifest`; and
7. `urn:nkf:0.6:schema:validation-result`.
...
The manifest's checker, authority, freshness-policy, schemas, and licensing bindings repeat identities for
their special roles but MUST resolve to the same path and digest in `files`.
...
`contracts/nkf/0.6/release-set.yaml` is the sole exact coverage, membership,
class, and mode enumeration for the NKF 0.6 complete set. It is a closed YAML
...
executable companion, evaluation policy, repository license, repository
...
members and modes declared by `contracts/nkf/0.6/release-set.yaml` under stable
...
0.6 is pre-stable. Before publication, release automation downloads the
...
control does not alter the NKF 0.6 format.
...
but NKF 0.6 does not define that policy. A consumer MUST NOT hide missing
...
The following remain deliberately unresolved in NKF 0.6:
...
profile decision is part of NKF 0.6 core.

## Minimal Example

A logical NKF 0.6 manifest:
...
| --- | --- | --- |
| NKF attempts to model every kind of knowledge immediately | The version becomes unusable and untestable | Keep 0.6 limited to evidence-backed Product and Technology profiles and add later roots from real needs |
