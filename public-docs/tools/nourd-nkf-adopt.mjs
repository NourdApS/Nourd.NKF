...
var FIXTURE_FILES = Object.freeze([
...
var PUBLIC_DOCUMENTATION_FILES = Object.freeze([
...
var RELEASE_ENTRIES = Object.freeze([
  ...
  { path: "dist/nourd-nkf-adopt.mjs", mode: 420 },
...
function rewriteOutboundLinks(text3, fromRelative, toRelative) {
  const fromDirectory = fromRelative.split("/").slice(0, -1);
  let fenced = false;
  return text3.split("\n").map((line) => {
    if (line.trim().startsWith("```")) {
    ...
    return line.replace(/\]\(([^()\s]+)\)/g, (whole, destination) => {
      ...
      return `](${rebased}${fragment === void 0 ? "" : `#${fragment}`})`;
    });
  }).join("\n");
}
...
async function gitTransitionPlan(git, projectRoot, transition, taskId) {
  ...
  const isLinkedWorktree = git.optional("rev-parse", "--git-dir") !== git.optional("rev-parse", "--git-common-dir");
