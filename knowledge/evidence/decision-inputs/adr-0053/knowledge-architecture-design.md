...
- **Authority Effect:** None until accepted through a Decision
...
## Proposed Or Accepted Design

This section is proposed. It is not accepted by the existence of this file or
by any later implementation performed before a Decision.
...
| `adopted` | A Decision selected the proposal | Governing Decision |
| `rejected` | A Decision declined the proposal | Governing Decision |
| `superseded` | A later Design or Decision replaced it | Replacement and relevant Decision |
...
authority state and is not a Design outcome. Historical `abandoned` wording
maps to `withdrawn` only when explicit withdrawal provenance exists.
...
| Design disposition | Native lifecycle | Native authority state |
| --- | --- | --- |
| Active | `living` | `draft` |
| Adopted | `immutable` | `accepted` |
| Rejected | `immutable` | `draft` |
| Superseded after adoption | `immutable` | `superseded` |
| Superseded before adoption | `immutable` | `draft` |
| Withdrawn | `immutable` | `draft` |

Rejected, pre-adoption Superseded, and Withdrawn proposals remain Draft in the
authority axis because their proposed meaning was never accepted. Their
terminal disposition prevents them from being mistaken for active work.
...
### Use Accepted As The Adopted Design Disposition

This matches colloquial wording but conflicts with the native record authority
state `accepted` and obscures whether a Decision selected the proposal.

Rejected. `adopted` names the proposal outcome.
