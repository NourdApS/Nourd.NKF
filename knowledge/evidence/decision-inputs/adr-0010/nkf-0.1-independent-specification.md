...
- **Independent governing inputs:** ADRs 0001 through 0009
- **Interoperability baseline:** Open Knowledge Format 0.2

> This is a proposed independent NKF realization. It preserves the accepted
> Studio NKF 0.1 source baseline and incorporates only the accepted NKF
> Decisions identified above. It is not accepted merely because its inputs
> are accepted, and it is not the public stable NKF 1.0 release.
...
The Studio NKF 0.1 source baseline is an accepted pre-stable format for
internal use and piloting with Shredwise and Nourd Studio. This independent
composite remains a proposal until its exact revision is accepted. Its `0.x`
version communicates that public governance and compatibility are not yet
stable. A validator result, Git commit, merge, file status, or tool output
...
accepted that exact Studio composite on 28 July 2026. That acceptance does not
accept this independent composite revision.
...
| `contract` | NKF bundle identity (`nkf.bundle`) |
