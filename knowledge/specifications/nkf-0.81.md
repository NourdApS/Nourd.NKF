---
id: nkf-0.81-specification
title: NKF 0.81 — Product And Technology Knowledge Format
type: specification
summary: NKF 0.81 is the successor to NKF 0.8 that makes adoption from the public governing repository truthful and self-contained. The recommended-release catalog and its closed channel vocabulary become accepted contract meaning with public prerelease and release values, a consumer resolves the catalog and archive over plain HTTPS with digest verification and no command-line tool, the adoption protocol states where the adopter is obtained, a closed registry of three volatile operating-system metadata files stops onboarding drift without weakening complete review, the checker recomputes the delta-review closure with impact propagation instead of trusting the recorded one, and live support slides to exactly 0.81 plus 0.8 with NKF 0.71 as stepping-stone history.
created_at: 2026-09-08T19:20:00Z
---

# NKF 0.81 — Product And Technology Knowledge Format

- **Predecessor canonical baseline:** `knowledge/specifications/nkf-0.8.md`
- **Predecessor canonical digest:** `6a5c571d6dcc1a00c52467095e8835458d829c67e5c9e72f9363fe66cec2982e`
- **Predecessor executable baseline:** `contracts/nkf/0.8/nkf.yaml`
- **Predecessor executable digest:** `6d162800ecb2a41fb9b6ead544cc0d5ecd4dc119cd72298138a4d17ccd822326`
- **Predecessor evaluation policy:** `contracts/nkf/0.8/freshness-policy.yaml`
- **Predecessor evaluation-policy digest:** `7cfb1b40a8b6f86543cb7b6016c6e9b2c1159d2a748c1915d0e9bf51bb82afd4`
- **Predecessor version-delta declaration:** `contracts/nkf/0.8/version-delta.yaml`
- **Predecessor version-delta digest:** `cc17f23a44afcf56b107f3e76e4d761779a4225bd2f8955c51d9781193c9c8b9`
- **Canonical destination:** `knowledge/specifications/nkf-0.81.md`
- **Executable companion destination:** `contracts/nkf/0.81/nkf.yaml`
- **Evaluation-policy destination:** `contracts/nkf/0.81/freshness-policy.yaml`
- **Version-delta declaration destination:** `contracts/nkf/0.81/version-delta.yaml`
- **Governing successor direction:** [ADR 0138](../decisions/0138-adopt-the-nkf-0-81-public-adoption-direction.md)
  and [ADR 0139](../decisions/0139-adopt-the-delta-closure-propagation-repair.md)
- **Independent governing inputs:** the exact NKF 0.8 authority set accepted
  by
  [ADR 0134](../decisions/0134-accept-the-nkf-0-8-authority-set.md)
  and confirmed for release by
  [ADR 0135](../decisions/0135-confirm-the-nkf-0-8-release-candidate.md),
  the public-repository direction adopted by
  [ADR 0136](../decisions/0136-adopt-the-public-repository-direction.md)
  with the catalog gap it recorded, the public-adoption direction adopted by
  [ADR 0138](../decisions/0138-adopt-the-nkf-0-81-public-adoption-direction.md)
  with every boundary confirmed by the Human Product Owner, the delta-closure
  propagation repair adopted by
  [ADR 0139](../decisions/0139-adopt-the-delta-closure-propagation-repair.md),
  the volatile-metadata onboarding drift recorded in
...
> This exact revision is the canonical NKF 0.81 specification when accepted
...
> and
> [ADR 0139](../decisions/0139-adopt-the-delta-closure-propagation-repair.md);
> governs repositories that deliberately declare NKF 0.81, and does not alter
> the immutable authority of NKF 0.1 through NKF 0.8.

The published NKF 0.8 complete set remains immutable at its exact archive,
authority, checker, adopter, and repository coordinates. Its governing
repository became public on `2026-09-08`, and four facts frozen in the 0.8
bytes then stopped being true or usable from outside: the released adopter
validates the recommendation catalog against private-channel literals that no
accepted meaning owns, so the catalog states a false visibility it cannot
correct without breaking every released adopter; the adopter reaches the
catalog and the archive only through the Github command-line tool; the public
documentation projection tells its reader the release is private; and the
adoption protocol never says where the adopter itself comes from. A fifth
defect is independent of visibility: the onboarding snapshot binds every
filesystem entry, so operating-system metadata rewritten by a file browser
between inspection and adoption fails an otherwise sound plan. A sixth was
found while asking how stale records had escaped review: the delta-review seal
computes its closure without the impact propagation this Specification already
requires, and the checker verifies containment of the recorded closure without
recomputing it.

This successor addresses all six without repairing any published byte in
place. The recommended-release catalog becomes the accepted contract
`nkf.recommended-release` with a closed channel vocabulary that includes public
prerelease and release values, so visibility is a fact the catalog states and
the contract validates rather than a constant tooling assumes. A consumer
resolves the catalog and the archive over plain HTTPS with the archive digest
and installed pin as the only trust anchors, and the adoption protocol states
first where to obtain the adopter and how to verify it. A closed registry of
...
format for durable governed knowledge. NKF 0.81 supports Product and Technology
...
NKF 0.81 is a pre-stable format version produced under the versioned contract
evolution accepted by [ADR 0076](../decisions/0076-versioned-contract-evolution.md).
The complete accepted NKF 0.8 authority set and lineage remain the
...
adopts only the derived versioned guidance distribution, the checked guidance
self-description, the enumerated whole-set guidance review and its audit, the
version-keyed accepted integration chain, and the window slide.
...
`nkf_version` `0.81`; earlier NKF versions remain immutable authority for
...
declares NKF 0.8, as required by the prepublication sequence. Its
frontmatter is therefore the exact stable native 0.8 source envelope. Before
candidate Adopt it is represented as non-authoritative candidate Evidence at
the exact future canonical source path rather than a 0.8 Specification
record, and the candidate executable companion, evaluation policy, and
version-delta declaration are carried only as non-authoritative prospective
artifacts. A later accepted Decision may bind these exact Markdown,
executable-companion, policy, and version-delta bytes plus one exact
producer-only promotion input at
`knowledge/evidence/release/nkf-0.81-producer-promotion.yaml`. That input is
a strict non-authoritative Evidence YAML artifact; it contains the exact
four authority bindings and the complete future native Specification record
declaration plus the exact destination
`.nourd/knowledge/records/nkf-0.81-specification.yaml`, including the
...
At both authorized producer stages, 0.81 Adopt verifies the exact accepted
...
candidate Evidence document representation, and creates the native 0.81
...
neutralization, and 0.81 declares no new succession. A separate
reviewer-completed `nkf.semantic-review-input` for the exact post-promotion
candidate is required at execution time and produces the native 0.81
confirmed baseline. Those stage reviews are delta reviews against the exact
promoted predecessor baseline under the accepted 0.8-to-0.81 version-delta
declaration — ordinary delta-proven promotions under the mechanism the
predecessor lineage established — and whole-root review remains the recovery
path. No ordinary consumer update can
invoke this producer-only promotion. Validation, migration, declaration
generation, or the promotion input cannot supply acceptance or derive the
...
second is ordinary public self-Adopt by the exact live NKF producer after
the immutable published archive has been acquired through the exact promoted
recommendation. Both stages use the same accepted promotion bytes, accepting
Decision, declaration destination, transactional postconditions, and
rollback rules; each uses a separately completed delta review for its exact
post-promotion candidate graph. After successful public promotion, a repeat
Adopt returns `current` only after it re-verifies the exact native 0.81
declaration, absence of the candidate Evidence representation, generated
integration, candidate graph, and confirmed baseline. It performs no
...
acceptance attempt retains its exact `historical_acceptance_attempt_lock`
unchanged: it remains digest-bound, graph-visible accepted history under the
closed lock this lineage carries, is not current release authority, and is
not a reusable Evidence exemption. Existing 0.5 records carrying the
historical `accepted_bootstrap_lock` or `prepublication_supersession_lock`
retain those exact predecessor-only declaration exceptions. Their exact
source bytes, identities, locks, and predecessor meaning remain immutable,
and the exceptions are unavailable to new 0.81-native records.
...
Technology-first NKF self-hosting exercise establish the Common, Product,
...
NKF 0.7 made semantic review carry-forward digest-bound and computable,
confined fresh review to the declared per-rule semantic delta with a
fail-closed delta claim, neutralized state-baked identity and stable paths
through explicit succession, made the post-audit technical-confirmation
Decision mandatory and audit-bound, and cut live support to the current
version plus one predecessor through
[ADR 0128](../decisions/0128-accept-the-revised-nkf-0-7-authority-set.md).
NKF 0.71 reconciled the portable-topology contradiction so prose and
executable state one neutral realizations layout explicitly, corrected the
...
NKF 0.81 defines Common contracts and two concrete Root Profiles: Product and
...
literal appearing there is a self-description and MUST state the version the
file serves. When a governed project contains a guidance file at a native
...
evaluation cannot supply. A guidance file with no frontmatter, no
`description`, or no version literal in its `description` emits nothing under
this rule.
...
NKF 0.81 live-implements exactly the current version plus one predecessor:
the 0.81 checker and adopter dispatch NKF 0.81 and NKF 0.8 and no earlier
...
and digest. A 0.81 tool encountering an out-of-window version MUST fail
...
updating an exact conformant NKF 0.8 repository to NKF 0.81 is
`non-breaking` and proceeds through the ordinary update, which rebinds the
contract set mechanically while preserving canonical Markdown bytes and
historical authority, applies no identity succession and no stable-path
migration, and converts the reviewed baseline to the digest-bound 0.81
baseline contract. Baseline conversion computes carry-forward per judgment:
a judgment carries only when the judged node revision, its basis digest, and
every rule it depends on are `identical` under the accepted 0.8-to-0.81
version-delta declaration; everything else enters the required fresh-review
set of the update review. The rules a judgment depends on are not
...
membership in the declaring judgment kind's list. The updater never invents,
discards, or reinterprets semantic review; a repository whose 0.8 baseline
is missing, outdated, disputed, ambiguous, or otherwise not ready is
ineligible for the update until a separate governed 0.8
knowledge-maintenance operation restores it. An exact NKF 0.81-to-0.81
refresh is `non-breaking`, requires no migration, and requires no
repository-owner approval because the governing format version does not
change.

NKF has one version namespace: the NKF format version. A bundle MUST declare
`nkf_version`. NKF 0.81 uses the unversioned canonical identities `nkf.bundle`,
...
identities. They are all governed by the one NKF version coordinate, `0.81`.
Profile identity is not an independent version coordinate.

The sole NKF 0.81 record definition includes explicit responsibility bindings.
...
migrated by their consumers, but MUST NOT be reported as native NKF 0.81
...
version `0.81` alone does not identify editorial changes to a draft.

NKF 0.81 uses OKF 0.2 as its interoperability baseline. A later OKF release
...
contracts. NKF 0.81 does not silently rename or reinterpret them. A controlled
...
that directly contains `.nourd/`. Native NKF 0.81 fixes:
...
| --- | --- |
| `nkf_version` | Constant NKF version `"0.81"` |
...
top-level fields fail closed. Native NKF 0.81 has no `product_record`,
...
becomes its document-node ID during migration. A new 0.81 Task is created with
...
NKF 0.81, outside canonical Markdown, and governed by the baseline contract
...
native 0.81 checker matches a supplied external observation by that exact ID
...
Every Product and Technology bundle using this successor NKF 0.81 revision
...
| `realizations/README.md` | One `navigation` non-record linking the consolidated current-system Realization and every represented Realization record below `realizations/items/` |
...
- `realizations/current-system.md` is the sole consolidated current-system
  Realization; and
- every represented Realization record below `realizations/items/` is
  linked exactly once from `realizations/README.md`.

The parent Task and Design indexes link their required child indexes
exactly once, and the realizations index links the current-system source
exactly once. Lifecycle indexes are
...
`tasks/deferred`, `tasks/cancelled`, `designs/active`, `designs/adopted`,
`designs/rejected`, `designs/superseded`, `designs/withdrawn`, and
`realizations/current` trees — moved exactly once to the corresponding
neutral `tasks/items/`, `designs/items/`, or `realizations/items/` path,
with declarations, navigation, and same-bundle links rewritten mechanically
in the same transaction and immutable record meaning untouched. Sources
carrying a predecessor-only lock, immutable record sources, and Evidence
...
by the version-free identifier
[`nkf-current-system`](../realizations/current-system.md), recorded with
succession provenance in the same governed act. This revision declares no
new identity succession and no new path migration. After that
neutralization, stable paths never move again: a lifecycle transition
changes only the declaration state and generated navigation and MUST NOT
move or rewrite the canonical Markdown source. New Task, Design, and
...
Live migration into NKF 0.81 exists only from NKF 0.8, per the support
window. The non-breaking 0.8-to-0.81 update preserves every canonical
Markdown byte, performs no stable-path migration and applies no identity
succession, rebinds the contract set mechanically, upgrades declarations to
the 0.81 shapes, converts the reviewed baseline to the digest-bound 0.81
baseline contract with computed per-judgment carry-forward under the
accepted 0.8-to-0.81 version-delta declaration, and regenerates navigation.
Preserved predecessor-only `legacy_lock`, `accepted_bootstrap_lock`,
`prepublication_supersession_lock`, and
...
act. Repositories declaring NKF 0.1 through NKF 0.71 are out of window: the
0.81 tool fails closed and names the exact published stepping-stone archive
...
unsupported baseline evidence produces `unknown` and blocks adoption. The
semantic reviewer's act does not imply acceptance of canonical Product or
Technology meaning. The updater never invents, carries, or discards a
judgment outside the computed rules.
...
prospective Evidence representation, creates the native 0.81 Specification
declaration at the supplied destination, applies no identity succession and
no path migration, and seals the exact post-promotion graph from a
separately completed delta review against the exact promoted predecessor
baseline, with whole-root review remaining the recovery path. The
predecessor-only locks are forbidden on that new 0.81 record.
...
Native 0.81 frontmatter is deliberately stable orientation. Mutable lifecycle,
...
a winner or rewrite either representation. Native 0.81 frontmatter MUST NOT
...
the exemption and requires the native 0.81 envelope. This narrow rule preserves
...
and current state
`immutable`/`accepted`. It cannot be allocated to a 0.81-native record.
...
state key in native 0.81 source emits
...
0.7 migration rewrote. Because the closed mapping collapses
...
Native NKF 0.81 interprets the Markdown source body using CommonMark 0.31.2.
...
Two further optional closed structures are native to NKF 0.81:
...
above, NKF 0.81 does not impose a narrower lexical format on the optional
...
Native NKF 0.81 defines no presentation-guidance field. Markdown is the default
...
Cross-bundle identity and typed relationships are deferred in NKF 0.81.
...
NKF 0.81 defines these record relationship types:
...
0.81. Every supported non-context, non-historical relationship consequence is
...
`0.81`. It binds bundle identity, selected profile, graph revision, policy
...
A deterministic Task state transition seals its own successor baseline
through the conclusion carry. Within one transition transaction, after the
declared state value changes under the accepted state machine and the
generated state indexes regenerate, the transition MUST reseal the baseline
over exactly that delta. The transitioned Task node's judgments MAY be
conclusion-carried only when the node's source digest and every judgment
basis digest are unchanged, the only changed revision input is the declared
state value, and every rule the judgment depends on is `identical` under the
accepted version-delta declaration; a conclusion-carried judgment
additionally binds the exact transition — the Task identity, the prior state,
and the new state — beside the performing-baseline provenance it already
carries. Every other judgment carries by ordinary digest identity. A
transition whose graph delta exceeds this closed vocabulary MUST fail before
mutation, with the ordinary review-and-seal path as recovery. The conclusion
supplies no semantic judgment and resolves no open uncertainty.
...
one of the literal claims `semantically-reviewed-whole-root`,
`semantically-reviewed-delta`, or `mechanically-concluded`, a non-empty set
of exact source-bound review observations, known limitations, and
`disputed: false`. Under the whole-root claim every judgment is `performed`.
Under the delta claim the confirmation additionally binds the exact computed
required-review closure and the exact performed set, and the checker admits
the claim only when the performed set contains the computed closure. The
checker MUST recompute that closure itself from the candidate graph, the
predecessor baseline, the evaluation policy's impact propagation, and the
accepted version-delta declaration, and MUST refuse a claim whose recorded
closure differs from the recomputed one, emitting
`freshness.claim.computed-closure-not-reproduced` and naming the missing
subjects. Containment against a recorded closure alone is not admission. Under
the `mechanically-concluded` claim every judgment is carried, the
confirmation binds the exact transition and the exact graph revision of the
predecessor baseline it concluded from, and the checker admits the claim
only when the graph delta from that bound predecessor baseline is exactly
the closed transition vocabulary; the recorded reviewer is the transition
operator, it performs zero judgments, and the nearest predecessor baseline
carrying a semantic claim remains the semantic provenance. Every graph
relationship category required by the selected profile is marked `reviewed`
or `inapplicable` with a source-bound basis. Empty or generic assertions do
not satisfy review evidence.
...
`contracts/nkf/0.81/version-delta.yaml`. It binds the current and predecessor
...
Freshness receipts use contract `nkf.freshness-receipt`, version `0.81`, and are
...
one exact supplied observation identity. NKF 0.81 supports no free-form
...
states the claimed original acceptance date. Native NKF 0.81 has no universal
...
through an explicit 0.81 freshness declaration, policy, purpose, and bound
...
`nkf_version: "0.81"` is the only version coordinate and artifact digests bind
exact revisions.
...
2. executable YAML with identity `nkf.extension`, `nkf_version: "0.81"`, the
...
Native NKF 0.81 accepts no concrete extension. Portable presentation guidance
...
The fixed `.nourd` layout in the bundle contract is native NKF 0.81, not a
...
NKF 0.81 is based on the official
...
```text
contracts/nkf/0.81/schemas/
  bundle.schema.json   # urn:nkf:0.81:schema:bundle
  record.schema.json   # urn:nkf:0.81:schema:record
...
```text
contracts/nkf/0.81/schemas/release-manifest.schema.json
                       # urn:nkf:0.81:schema:release-manifest
```
...
| `guidance.self-description.version-mismatch` | error |
...
| `freshness.baseline.conclusion.delta-exceeded` | error | conformance |
| `freshness.baseline.conclusion.carry-invalid` | error | conformance |
...
| `freshness.claim.computed-closure-not-reproduced` | error | conformance |
...
closed UTF-8 JSON `nkf.validation-result` object with `nkf_version: "0.81"`.
...
`nkf.validation-snapshot` JSON with `nkf_version: "0.81"` and one entry per
...
snapshot, its core artifact bindings match the current accepted NKF 0.81
...
process. NKF 0.81 does not infer a hidden acceptance change, poll an authority,
...
| Task transition | One exact Task document, its declared state, required Completion or Cancellation Result, generated state navigation, affected source digests, governed references, and the seal-completed successor baseline whose graph delta from the bound predecessor baseline is exactly the closed transition vocabulary; a transition delta exceeding that vocabulary fails before mutation, and dirty or unsupported Git orchestration remains a separate truthful operational result. |
...
none of this: derivation proves that every emitted copy states its version
truthfully and proves nothing about whether the sentence it carries is still
the current rule.
...
### Volatile Metadata Registry

Initial onboarding captures every filesystem entry of the project outside
top-level `.git`, binds the capture in the sealed plan, and refuses to apply a
plan whose project has drifted. That boundary is complete review, and it is
kept. Exactly three entries are recognized as volatile operating-system
metadata: regular files whose basename is exactly `.DS_Store`, `Thumbs.db`,
or `desktop.ini`, at any depth. The inspection manifest records each such
entry with the classification `volatile` so the participating reviewer sees
it; the snapshot digest bound by the plan excludes its bytes and its presence;
and preflight tolerates a registered basename appearing, changing, or
disappearing between seal and application. Every other entry keeps fail-closed
drift protection unchanged.

The registry is closed. It admits no glob, no project-declared addition, no
content inspection, and no inference from `.gitignore` or any other
project-controlled file. An entry with a registered basename that is a symbolic
link, directory, or special file is not volatile and is captured and protected
like any other entry. Onboarding never deletes a project-owned file. Any change
to the registry follows the governed pre-stable change process and ships as a
new version.

## Release Distribution

NKF 0.81 uses one content-addressed archive attached to a Github Release in
...
They are not NKF versions. `nkf_version: "0.81"` remains the only format
version.

### Release Manifest

The archive contains exactly one UTF-8 JSON `release-manifest.json` and one
UTF-8 YAML `contracts/nkf/0.81/release-set.yaml`. The release manifest has
contract identity `nkf.release-manifest` and `nkf_version: "0.81"`; the release
...
`contracts/nkf/0.81/release-set.yaml` is the sole exact coverage, membership,
class, and mode enumeration for the NKF 0.81 complete set. It is a closed YAML
...
Every version-bearing member of the authoring protocol, onboarding protocol,
release protocol, adoption protocol, portable skill, and registered
host-adapter instruction classes MUST be produced by deriving it from the
single version-neutral authored source with this version injected, never by
copying a predecessor member. Regenerating those members from that source and
this version alone MUST reproduce the exact bytes the release set enumerates,
and a member whose committed bytes differ from the derivation is invalid.
...
members and modes declared by `contracts/nkf/0.81/release-set.yaml` under stable
...
The governing repository publishes one recommended-release catalog at the
fixed path `release/recommended.json` on its default branch. The catalog is
the selection channel: it names the exact version, archive digest, release
commit, checker digest, adopter digest, and authority digests a consumer
should adopt. It is not a trust anchor. The verified full archive digest and
the installed consumer pin remain the trust anchors, and a catalog that
disagrees with either is refused.

The catalog is the closed contract `nkf.recommended-release`, validated by
the derived `recommended-release.schema.json`. Its fields are `contract`,
`nkf_version`, `state`, `channel`, `compatibility`, `archive`,
`source_commit`, `checker_sha256`, `authority`, `adopter_sha256`,
`release`, and `supported_root_profiles`, exactly as the executable
companion declares. The channel vocabulary is closed and owned here:

| Channel | Release visibility | Prerelease | Meaning |
| --- | --- | --- | --- |
| `internal-exact-candidate` | `unpublished` | `true` | The isolated exact-candidate exercise; `archive.url` is `null`. |
| `internal-private-github-prerelease` | `private` | `true` | Historical: the channel every version through NKF 0.8 was published under while the governing repository was private. A 0.8 catalog may still state it; a 0.81 catalog MUST NOT. |
| `public-github-prerelease` | `public` | `true` | A pre-stable version published as a Github prerelease from the public governing repository. NKF 0.81 is published under this channel. |
| `public-github-release` | `public` | `false` | A version published as a Github release from the public governing repository. |

A consumer resolves the catalog over plain HTTPS from the governing
repository's raw default-branch content and the archive from `archive.url`,
which MUST equal the canonical asset locator
`https://github.com/<repository>/releases/download/<tag>/<asset_name>` for
the declared repository, tag, and asset name. No command-line tool, session, or
credential is required or consulted. A response other than success, a body
whose SHA-256 differs from `archive.sha256`, a catalog that fails the Schema,
names a channel outside the vocabulary, or states a visibility or prerelease
flag inconsistent with its channel, or an `archive.url` that does not match
the canonical form is refused before any project mutation, with the explicit
offline path — a locally supplied catalog and archive — as the recovery. The
adopter is obtained from the public documentation projection at
`tools/nourd-nkf-adopt.mjs` and verified against `reference/publication.json`
and the catalog's `adopter_sha256` before it is run; the adoption protocol
states that step first.

Repository visibility and the catalog channel are operational facts the
catalog states; they are validated against this contract, never inferred from
tooling constants.
...
The release is prepared as a draft and published under the channel the
catalog states: as a prerelease for `public-github-prerelease`, as a release
for `public-github-release`. NKF 0.81 is published as a prerelease because it
is pre-stable. Before publication, release automation downloads the
...
The governing repository is public, and public distribution is the
recommended-release catalog above. Repository visibility does not alter the
NKF 0.81 format; it is a fact the catalog states and this contract validates.
...
but NKF 0.81 does not define that policy. A consumer MUST NOT hide missing
...
The following remain deliberately unresolved in NKF 0.81:
...
- public governance, contribution process, trademark position, and
  Company-side rights evidence; public distribution policy is resolved by the
  recommended-release catalog and the public governing repository;
...
- review-quality measurement and multi-writer semantics, explicitly deferred
  by the Human Product Owner without a version commitment.
...
profile decision is part of NKF 0.81 core.

## Minimal Example

A logical NKF 0.81 manifest:
...
| --- | --- | --- |
| NKF attempts to model every kind of knowledge immediately | The version becomes unusable and untestable | Keep 0.81 limited to evidence-backed Product and Technology profiles and add later roots from real needs |
