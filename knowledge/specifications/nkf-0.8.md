---
id: nkf-0.8-specification
title: NKF 0.8 — Product And Technology Knowledge Format
type: specification
summary: NKF 0.8 is the successor to NKF 0.71 that ends the recurring stale-guidance defect class at its source. Every version-bearing guidance copy is derived from one version-neutral authored source with the version injected instead of carried forward by hand, a guidance file's own frontmatter self-description becomes a checked conformance position, the pre-cut whole-set guidance review and its independent audit are enforced against the deterministically enumerated release set, and live support slides to exactly 0.8 plus 0.71 with NKF 0.7 as stepping-stone history.
created_at: 2026-08-19T01:00:00Z
---

# NKF 0.8 — Product And Technology Knowledge Format

- **Predecessor canonical baseline:** `knowledge/specifications/nkf-0.71.md`
- **Predecessor canonical digest:** `ec6d7fd733a989eb86580bd3a9d77405d3c02d5402429190ad8d171c4414ae98`
- **Predecessor executable baseline:** `contracts/nkf/0.71/nkf.yaml`
- **Predecessor executable digest:** `65b5bec3dd16cd1872216b2dc17acbe77f1a363e76112f675a271e337cf6ec1a`
- **Predecessor evaluation policy:** `contracts/nkf/0.71/freshness-policy.yaml`
- **Predecessor evaluation-policy digest:** `20c7b5f4e34f88da6c8365a59cee913020263c2c4ab36796602a2a7cb0ab1ae4`
- **Predecessor version-delta declaration:** `contracts/nkf/0.71/version-delta.yaml`
- **Predecessor version-delta digest:** `21089afc8155a98410960e2fa7cf19344e1e5ca010095a0f7975dc783d730c01`
- **Canonical destination:** `knowledge/specifications/nkf-0.8.md`
- **Executable companion destination:** `contracts/nkf/0.8/nkf.yaml`
- **Evaluation-policy destination:** `contracts/nkf/0.8/freshness-policy.yaml`
- **Version-delta declaration destination:** `contracts/nkf/0.8/version-delta.yaml`
- **Governing successor direction:** [ADR 0133](../decisions/0133-adopt-the-nkf-0-8-generated-distribution-direction.md)
- **Independent governing inputs:** the exact NKF 0.71 authority set accepted
  by
  ...
  the generated-distribution direction adopted by
  [ADR 0133](../decisions/0133-adopt-the-nkf-0-8-generated-distribution-direction.md)
  with every planning boundary confirmed by the Human Product Owner, the
  already-accepted whole-set guidance review of
  [ADR 0097](../decisions/0097-full-set-guidance-review-and-enumeration.md),
  the extracted-byte finding recorded in the
  [NKF 0.71 guidance-review finding](../evidence/audits/nkf-033-nkf-0-71-guidance-review-finding.md),
...
> This exact revision is the canonical NKF 0.8 specification when accepted
...
> [ADR 0133](../decisions/0133-adopt-the-nkf-0-8-generated-distribution-direction.md);
> governs repositories that deliberately declare NKF 0.8, and does not alter
> the immutable authority of NKF 0.1 through NKF 0.71.

The published NKF 0.71 complete set remains immutable at its exact archive,
authority, checker, adopter, and repository coordinates — including one stale
version label inside it, which this successor corrects and never repairs in
place. That label is the recorded instance of a defect class with two
established causes, each of which recurs by construction. The versioned
guidance distribution was duplicated by hand rather than derived, so cutting a
version copied the predecessor tree and changed exactly the marker line the
checker enforces while every other version-bearing sentence rotted silently.
And the release protocol's independent-audit step still directed verification
against the rule-diff standard that
[ADR 0097](../decisions/0097-full-set-guidance-review-and-enumeration.md) had
already superseded with the whole-set review, so a diff-scoped review passed an
audit told to check the superseded rule.

This successor removes both causes. Every version-bearing guidance copy is
derived from one version-neutral authored source with the version injected, no
emitted copy is ever an input to another, and the source may state no literal
NKF version at all. A guidance file's own frontmatter self-description becomes
a checked conformance position through one added registry rule, so the exact
position that carried the published defect fails closed in every repository
that declares this version rather than only in the producer. The whole-set
pre-cut guidance review and the independent audit that verifies it are bound to
the deterministically enumerated release set. Live support slides to exactly
the current version plus one predecessor — NKF 0.8 plus NKF 0.71 — with NKF 0.7
stepping through the published 0.71 archive. It adds no deferred Product
capability and does not rewrite any published 0.71 member.

## Purpose

The **Nourd Knowledge Format (NKF)** is a human-readable, machine-verifiable
format for durable governed knowledge. NKF 0.8 supports Product and Technology
...
NKF 0.8 is a pre-stable format version produced under the versioned contract
evolution accepted by [ADR 0076](../decisions/0076-versioned-contract-evolution.md).
The complete accepted NKF 0.71 authority set and lineage remain the
...
`nkf_version` `0.8`; earlier NKF versions remain immutable authority for
...
declares NKF 0.71, as required by the prepublication sequence. Its
frontmatter is therefore the exact stable native 0.71 source envelope. Before
candidate Adopt it is represented as non-authoritative candidate Evidence at
the exact future canonical source path rather than a 0.71 Specification
...
`knowledge/evidence/release/nkf-0.8-producer-promotion.yaml`. That input is
...
`.nourd/knowledge/records/nkf-0.8-specification.yaml`, including the
...
At both authorized producer stages, 0.8 Adopt verifies the exact accepted
...
candidate Evidence document representation, and creates the native 0.8
...
neutralization, and 0.8 declares no new succession. A separate
reviewer-completed `nkf.semantic-review-input` for the exact post-promotion
candidate is required at execution time and produces the native 0.8
confirmed baseline. Those stage reviews are delta reviews against the exact
promoted predecessor baseline under the accepted 0.71-to-0.8 version-delta
...
Adopt returns `current` only after it re-verifies the exact native 0.8
...
and the exceptions are unavailable to new 0.8-native records.
...
[ADR 0131](../decisions/0131-accept-the-nkf-0-71-authority-set.md).
This NKF 0.8 revision preserves that complete predecessor meaning and adds
...
NKF 0.8 defines Common contracts and two concrete Root Profiles: Product and
...
NKF 0.8 live-implements exactly the current version plus one predecessor:
the 0.8 checker and adopter dispatch NKF 0.8 and NKF 0.71 and no earlier
...
and digest. A 0.8 tool encountering an out-of-window version MUST fail
...
updating an exact conformant NKF 0.71 repository to NKF 0.8 is
...
migration, and converts the reviewed baseline to the digest-bound 0.8
...
every rule it depends on are `identical` under the accepted 0.71-to-0.8
...
discards, or reinterprets semantic review; a repository whose 0.71 baseline
is missing, outdated, disputed, ambiguous, or otherwise not ready is
ineligible for the update until a separate governed 0.71
knowledge-maintenance operation restores it. An exact NKF 0.8-to-0.8
...
`nkf_version`. NKF 0.8 uses the unversioned canonical identities `nkf.bundle`,
...
identities. They are all governed by the one NKF version coordinate, `0.8`.
Profile identity is not an independent version coordinate.

The sole NKF 0.8 record definition includes explicit responsibility bindings.
...
migrated by their consumers, but MUST NOT be reported as native NKF 0.8
...
version `0.8` alone does not identify editorial changes to a draft.

NKF 0.8 uses OKF 0.2 as its interoperability baseline. A later OKF release
...
contracts. NKF 0.8 does not silently rename or reinterpret them. A controlled
...
that directly contains `.nourd/`. Native NKF 0.8 fixes:
...
| --- | --- |
| `nkf_version` | Constant NKF version `"0.8"` |
...
top-level fields fail closed. Native NKF 0.8 has no `product_record`,
...
becomes its document-node ID during migration. A new 0.8 Task is created with
...
NKF 0.8, outside canonical Markdown, and governed by the baseline contract
...
native 0.8 checker matches a supplied external observation by that exact ID
...
Every Product and Technology bundle using this successor NKF 0.8 revision
...
Live migration into NKF 0.8 exists only from NKF 0.71, per the support
window. The non-breaking 0.71-to-0.8 update preserves every canonical
...
the 0.8 shapes, converts the reviewed baseline to the digest-bound 0.8
baseline contract with computed per-judgment carry-forward under the
accepted 0.71-to-0.8 version-delta declaration, and regenerates navigation.
...
prospective Evidence representation, creates the native 0.8 Specification
...
predecessor-only locks are forbidden on that new 0.8 record.
...
Native 0.8 frontmatter is deliberately stable orientation. Mutable lifecycle,
...
a winner or rewrite either representation. Native 0.8 frontmatter MUST NOT
...
the exemption and requires the native 0.8 envelope. This narrow rule preserves
...
and current state
`immutable`/`accepted`. It cannot be allocated to a 0.8-native record.
...
state key in native 0.8 source emits
...
Native NKF 0.8 interprets the Markdown source body using CommonMark 0.31.2.
...
Two further optional closed structures are native to NKF 0.8:
...
above, NKF 0.8 does not impose a narrower lexical format on the optional
...
Native NKF 0.8 defines no presentation-guidance field. Markdown is the default
...
Cross-bundle identity and typed relationships are deferred in NKF 0.8.
...
NKF 0.8 defines these record relationship types:
...
```json
{"contract":"nkf.graph-revision","nkf_version":"0.8","bundle":"<bundle-id>","profile":"<profile-id>","nodes":[{"node":<node-reference>,"revision":"<hex>"}],"edges":[<normalized-authored-edge>],"external_dependencies":[<normalized-external-dependency>],"authority_inputs":[<normalized-authority-input>],"policy":{"id":"nkf.freshness-policy.0.8","sha256":"<hex>"}}
```
...
NKF 0.8 distributes exactly one immutable policy named
`nkf.freshness-policy.0.8` at
`contracts/nkf/0.8/freshness-policy.yaml`. Its release manifest binds the exact
...
0.8. Every supported non-context, non-historical relationship consequence is
...
`0.8`. It binds bundle identity, selected profile, graph revision, policy
...
the claim only when the performed set contains the computed closure. Under
...
`contracts/nkf/0.8/version-delta.yaml`. It binds the current and predecessor
...
Freshness receipts use contract `nkf.freshness-receipt`, version `0.8`, and are
...
one exact supplied observation identity. NKF 0.8 supports no free-form
...
states the claimed original acceptance date. Native NKF 0.8 has no universal
...
through an explicit 0.8 freshness declaration, policy, purpose, and bound
...
`nkf_version: "0.8"` is the only version coordinate and artifact digests bind
exact revisions.
...
2. executable YAML with identity `nkf.extension`, `nkf_version: "0.8"`, the
...
Native NKF 0.8 accepts no concrete extension. Portable presentation guidance
...
The fixed `.nourd` layout in the bundle contract is native NKF 0.8, not a
...
NKF 0.8 is based on the official
...
```text
contracts/nkf/0.8/schemas/
  bundle.schema.json   # urn:nkf:0.8:schema:bundle
  record.schema.json   # urn:nkf:0.8:schema:record
  validation-result.schema.json
                       # urn:nkf:0.8:schema:validation-result
  graph-baseline.schema.json
                       # urn:nkf:0.8:schema:graph-baseline
  freshness-receipt.schema.json
                       # urn:nkf:0.8:schema:freshness-receipt
  freshness-policy.schema.json
                       # urn:nkf:0.8:schema:freshness-policy
```

The `0.8` component is the one NKF version coordinate. Each schema carries
...
contracts/nkf/0.8/schemas/release-manifest.schema.json
                       # urn:nkf:0.8:schema:release-manifest
```
...
closed UTF-8 JSON `nkf.validation-result` object with `nkf_version: "0.8"`.
...
`nkf.validation-snapshot` JSON with `nkf_version: "0.8"` and one entry per
...
snapshot, its core artifact bindings match the current accepted NKF 0.8
...
process. NKF 0.8 does not infer a hidden acceptance change, poll an authority,
...
NKF 0.8 uses one content-addressed archive attached to a Github Release in
...
They are not NKF versions. `nkf_version: "0.8"` remains the only format
version.

### Release Manifest

The archive contains exactly one UTF-8 JSON `release-manifest.json` and one
UTF-8 YAML `contracts/nkf/0.8/release-set.yaml`. The release manifest has
contract identity `nkf.release-manifest` and `nkf_version: "0.8"`; the release
...
- `knowledge/specifications/nkf-0.8.md`; and
- `contracts/nkf/0.8/nkf.yaml`.

`freshness_policy` binds identity `nkf.freshness-policy.0.8`, exact path
`contracts/nkf/0.8/freshness-policy.yaml`, and its SHA-256 digest.

`version_delta` binds contract `nkf.version-delta`, exact path
`contracts/nkf/0.8/version-delta.yaml`, and its SHA-256 digest.

`schemas` contains exactly seven entries in exact identity order:

1. `urn:nkf:0.8:schema:bundle`;
2. `urn:nkf:0.8:schema:record`;
3. `urn:nkf:0.8:schema:graph-baseline`;
4. `urn:nkf:0.8:schema:freshness-receipt`;
5. `urn:nkf:0.8:schema:freshness-policy`;
6. `urn:nkf:0.8:schema:release-manifest`; and
7. `urn:nkf:0.8:schema:validation-result`.
...
`contracts/nkf/0.8/release-set.yaml` is the sole exact coverage, membership,
class, and mode enumeration for the NKF 0.8 complete set. It is a closed YAML
...
members and modes declared by `contracts/nkf/0.8/release-set.yaml` under stable
...
0.8 is pre-stable. Before publication, release automation downloads the
...
control does not alter the NKF 0.8 format.
...
but NKF 0.8 does not define that policy. A consumer MUST NOT hide missing
...
The following remain deliberately unresolved in NKF 0.8:
...
profile decision is part of NKF 0.8 core.

## Minimal Example

A logical NKF 0.8 manifest:
...
| --- | --- | --- |
| NKF attempts to model every kind of knowledge immediately | The version becomes unusable and untestable | Keep 0.8 limited to evidence-backed Product and Technology profiles and add later roots from real needs |
