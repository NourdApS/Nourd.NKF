...
title: NKF 0.71 — Product And Technology Knowledge Format
type: specification
summary: NKF 0.71 is the deliberately small corrective successor to NKF 0.7. It reconciles the portable-topology contradiction so prose and executable state one neutral realizations layout explicitly, corrects the evaluation-policy version labels, makes the deterministic Task conclusion seal-completing so a concluded tip carries equal baseline and candidate graph revisions with zero fresh review, slides live support to exactly 0.71 plus 0.7 with NKF 0.6 as stepping-stone history, and is the first version whose producer promotion must be proven through the digest-bound delta claim alone.
created_at: 2026-08-17T18:00:00Z
---

# NKF 0.71 — Product And Technology Knowledge Format

- **Predecessor canonical baseline:** `knowledge/specifications/nkf-0.7.md`
- **Predecessor canonical digest:** `6d1c904608f57446939f9442d46aa3b735e34fce75ddfba24fabbea8416f0992`
- **Predecessor executable baseline:** `contracts/nkf/0.7/nkf.yaml`
- **Predecessor executable digest:** `9175c86b82100b84e983afbc06f89ff79c666bd8cd0626ee7892688e6551cc49`
- **Predecessor evaluation policy:** `contracts/nkf/0.7/freshness-policy.yaml`
- **Predecessor evaluation-policy digest:** `7bee48aac0fef1b1cc968efa25f2a6fb2205c77a0ec3fcf6c82ec17cdd5eeee0`
- **Predecessor version-delta declaration:** `contracts/nkf/0.7/version-delta.yaml`
- **Predecessor version-delta digest:** `dcc65c82512c73a204223a3bac990c951f44e995d5859426479042dbef8fcb37`
- **Canonical destination:** `knowledge/specifications/nkf-0.71.md`
- **Executable companion destination:** `contracts/nkf/0.71/nkf.yaml`
- **Evaluation-policy destination:** `contracts/nkf/0.71/freshness-policy.yaml`
- **Version-delta declaration destination:** `contracts/nkf/0.71/version-delta.yaml`
- **Governing successor direction:** [ADR 0130](../decisions/0130-adopt-the-nkf-0-71-corrective-successor-direction.md)
- **Independent governing inputs:** the exact NKF 0.7 authority set accepted
  by
  ...
  the corrective successor direction adopted by
  [ADR 0130](../decisions/0130-adopt-the-nkf-0-71-corrective-successor-direction.md)
  with every planning question resolved by the Human Product Owner, the
  sixth-audit findings recorded in the
...
> This exact revision is the canonical NKF 0.71 specification when accepted
...
> [ADR 0130](../decisions/0130-adopt-the-nkf-0-71-corrective-successor-direction.md);
> governs repositories that deliberately declare NKF 0.71, and does not alter
> the immutable authority of NKF 0.1 through NKF 0.7.

The published NKF 0.7 complete set remains immutable at its exact archive,
authority, checker, adopter, and repository coordinates. The sixth
independent audit judged the accepted 0.7 Specification fit-with-caveats and
recorded defects that immutable bytes cannot repair in place: the topology
chapter contradicts the neutralization chapter while the executable
companion silently omits the contested requirement, the evaluation-policy
section carries copy-forward version labels, and the deterministic Task
conclusion changes the knowledge graph after the delivering Task's last
seal, so every delivery lands one mechanical step stale. This corrective
successor reconciles the topology in prose and executable explicitly,
corrects every version label, makes the deterministic Task conclusion
seal-completing with zero fresh review under a closed mechanical carry,
slides live support to exactly the current version plus one predecessor —
NKF 0.71 plus NKF 0.7 — and is the first version whose producer promotion
must be proven through the digest-bound delta claim its predecessor
delivered. It adds no deferred Product capability and does not rewrite any
published 0.7 member.

## Purpose

The **Nourd Knowledge Format (NKF)** is a human-readable, machine-verifiable
format for durable governed knowledge. NKF 0.71 supports Product and Technology
...
NKF 0.71 is a pre-stable format version produced under the versioned contract
evolution accepted by [ADR 0076](../decisions/0076-versioned-contract-evolution.md).
The complete accepted NKF 0.7 authority set and lineage remain the
...
[ADR 0130](../decisions/0130-adopt-the-nkf-0-71-corrective-successor-direction.md)
adopts only the corrective topology reconciliation, version-label
correction, seal-completing conclusion, window slide, and
projection-regeneration direction.
...
`nkf_version` `0.71`; earlier NKF versions remain immutable authority for
...
declares NKF 0.7, as required by the prepublication sequence. Its
frontmatter is therefore the exact stable native 0.7 source envelope. Before
candidate Adopt it is represented as non-authoritative candidate Evidence at
the exact future canonical source path rather than a 0.7 Specification
...
`knowledge/evidence/release/nkf-0.71-producer-promotion.yaml`. That input is
...
`.nourd/knowledge/records/nkf-0.71-specification.yaml`, including the
...
At both authorized producer stages, 0.71 Adopt verifies the exact accepted
...
candidate Evidence document representation, and creates the native 0.71
...
neutralization, and 0.71 declares no new succession. A separate
reviewer-completed `nkf.semantic-review-input` for the exact post-promotion
candidate is required at execution time and produces the native 0.71
confirmed baseline. Those stage reviews are delta reviews against the exact
promoted predecessor baseline under the accepted 0.7-to-0.71 version-delta
declaration — the first promotions in this lineage proven through the delta
claim, discharging the bootstrap promise the predecessor recorded — and
whole-root review remains the recovery path. No ordinary consumer update can
...
Adopt returns `current` only after it re-verifies the exact native 0.71
...
and the exceptions are unavailable to new 0.71-native records.
...
[ADR 0128](../decisions/0128-accept-the-revised-nkf-0-7-authority-set.md).
This NKF 0.71 revision preserves that complete predecessor meaning and adds
only the corrective topology reconciliation, version-label correction,
seal-completing conclusion, window slide, and projection-regeneration
direction adopted by
[ADR 0130](../decisions/0130-adopt-the-nkf-0-71-corrective-successor-direction.md).
...
NKF 0.71 defines Common contracts and two concrete Root Profiles: Product and
...
NKF 0.71 live-implements exactly the current version plus one predecessor:
the 0.71 checker and adopter dispatch NKF 0.71 and NKF 0.7 and no earlier
...
and digest. A 0.71 tool encountering an out-of-window version MUST fail
...
updating an exact conformant NKF 0.7 repository to NKF 0.71 is
...
migration, and converts the reviewed baseline to the digest-bound 0.71
...
every rule it depends on are `identical` under the accepted 0.7-to-0.71
...
discards, or reinterprets semantic review; a repository whose 0.7 baseline
is missing, outdated, disputed, ambiguous, or otherwise not ready is
ineligible for the update until a separate governed 0.7
knowledge-maintenance operation restores it. An exact NKF 0.71-to-0.71
...
`nkf_version`. NKF 0.71 uses the unversioned canonical identities `nkf.bundle`,
...
identities. They are all governed by the one NKF version coordinate, `0.71`.
Profile identity is not an independent version coordinate.

The sole NKF 0.71 record definition includes explicit responsibility bindings.
...
migrated by their consumers, but MUST NOT be reported as native NKF 0.71
...
version `0.71` alone does not identify editorial changes to a draft.

NKF 0.71 uses OKF 0.2 as its interoperability baseline. A later OKF release
...
contracts. NKF 0.71 does not silently rename or reinterpret them. A controlled
...
that directly contains `.nourd/`. Native NKF 0.71 fixes:
...
| --- | --- |
| `nkf_version` | Constant NKF version `"0.71"` |
...
top-level fields fail closed. Native NKF 0.71 has no `product_record`,
...
becomes its document-node ID during migration. A new 0.71 Task is created with
...
```yaml
knowledge_graph:
  policy: nkf.freshness-policy.0.71
...
NKF 0.71, outside canonical Markdown, and governed by the baseline contract
...
native 0.71 checker matches a supplied external observation by that exact ID
...
Every Product and Technology bundle using this successor NKF 0.71 revision
...
Live migration into NKF 0.71 exists only from NKF 0.7, per the support
window. The non-breaking 0.7-to-0.71 update preserves every canonical
...
the 0.71 shapes, converts the reviewed baseline to the digest-bound 0.71
baseline contract with computed per-judgment carry-forward under the
accepted 0.7-to-0.71 version-delta declaration, and regenerates navigation.
...
0.71 tool fails closed and names the exact published stepping-stone archive
...
prospective Evidence representation, creates the native 0.71 Specification
...
predecessor-only locks are forbidden on that new 0.71 record.
...
Native 0.71 frontmatter is deliberately stable orientation. Mutable lifecycle,
...
a winner or rewrite either representation. Native 0.71 frontmatter MUST NOT
...
the exemption and requires the native 0.71 envelope. This narrow rule preserves
...
and current state
`immutable`/`accepted`. It cannot be allocated to a 0.71-native record.
...
0.71-era migration mechanism preserved as immutable history.
...
state key in native 0.71 source emits
...
Native NKF 0.71 interprets the Markdown source body using CommonMark 0.31.2.
...
Two further optional closed structures are native to NKF 0.71:
...
above, NKF 0.71 does not impose a narrower lexical format on the optional
...
Native NKF 0.71 defines no presentation-guidance field. Markdown is the default
...
Cross-bundle identity and typed relationships are deferred in NKF 0.71.
...
NKF 0.71 defines these record relationship types:
...
```json
{"contract":"nkf.graph-revision","nkf_version":"0.71","bundle":"<bundle-id>","profile":"<profile-id>","nodes":[{"node":<node-reference>,"revision":"<hex>"}],"edges":[<normalized-authored-edge>],"external_dependencies":[<normalized-external-dependency>],"authority_inputs":[<normalized-authority-input>],"policy":{"id":"nkf.freshness-policy.0.71","sha256":"<hex>"}}
```
...
NKF 0.71 distributes exactly one immutable policy named
`nkf.freshness-policy.0.71` at
`contracts/nkf/0.71/freshness-policy.yaml`. Its release manifest binds the exact
...
0.71. Every supported non-context, non-historical relationship consequence is
...
`0.71`. It binds bundle identity, selected profile, graph revision, policy
...
`contracts/nkf/0.71/version-delta.yaml`. It binds the current and predecessor
...
Freshness receipts use contract `nkf.freshness-receipt`, version `0.71`, and are
...
one exact supplied observation identity. NKF 0.71 supports no free-form
...
states the claimed original acceptance date. Native NKF 0.71 has no universal
...
through an explicit 0.71 freshness declaration, policy, purpose, and bound
...
`nkf_version: "0.71"` is the only version coordinate and artifact digests bind
exact revisions.
...
2. executable YAML with identity `nkf.extension`, `nkf_version: "0.71"`, the
...
Native NKF 0.71 accepts no concrete extension. Portable presentation guidance
...
The fixed `.nourd` layout in the bundle contract is native NKF 0.71, not a
...
NKF 0.71 is based on the official
...
```text
contracts/nkf/0.71/schemas/
  bundle.schema.json   # urn:nkf:0.71:schema:bundle
  record.schema.json   # urn:nkf:0.71:schema:record
  validation-result.schema.json
                       # urn:nkf:0.71:schema:validation-result
  graph-baseline.schema.json
                       # urn:nkf:0.71:schema:graph-baseline
  freshness-receipt.schema.json
                       # urn:nkf:0.71:schema:freshness-receipt
  freshness-policy.schema.json
                       # urn:nkf:0.71:schema:freshness-policy
```

The `0.71` component is the one NKF version coordinate. Each schema carries
...
contracts/nkf/0.71/schemas/release-manifest.schema.json
                       # urn:nkf:0.71:schema:release-manifest
```
...
closed UTF-8 JSON `nkf.validation-result` object with `nkf_version: "0.71"`.
...
`nkf.validation-snapshot` JSON with `nkf_version: "0.71"` and one entry per
...
snapshot, its core artifact bindings match the current accepted NKF 0.71
...
process. NKF 0.71 does not infer a hidden acceptance change, poll an authority,
...
NKF 0.71 uses one content-addressed archive attached to a Github Release in
...
They are not NKF versions. `nkf_version: "0.71"` remains the only format
version.

### Release Manifest

The archive contains exactly one UTF-8 JSON `release-manifest.json` and one
UTF-8 YAML `contracts/nkf/0.71/release-set.yaml`. The release manifest has
contract identity `nkf.release-manifest` and `nkf_version: "0.71"`; the release
...
- `knowledge/specifications/nkf-0.71.md`; and
- `contracts/nkf/0.71/nkf.yaml`.

`freshness_policy` binds identity `nkf.freshness-policy.0.71`, exact path
`contracts/nkf/0.71/freshness-policy.yaml`, and its SHA-256 digest.

`version_delta` binds contract `nkf.version-delta`, exact path
`contracts/nkf/0.71/version-delta.yaml`, and its SHA-256 digest.

`schemas` contains exactly seven entries in exact identity order:

1. `urn:nkf:0.71:schema:bundle`;
2. `urn:nkf:0.71:schema:record`;
3. `urn:nkf:0.71:schema:graph-baseline`;
4. `urn:nkf:0.71:schema:freshness-receipt`;
5. `urn:nkf:0.71:schema:freshness-policy`;
6. `urn:nkf:0.71:schema:release-manifest`; and
7. `urn:nkf:0.71:schema:validation-result`.
...
`contracts/nkf/0.71/release-set.yaml` is the sole exact coverage, membership,
class, and mode enumeration for the NKF 0.71 complete set. It is a closed YAML
...
members and modes declared by `contracts/nkf/0.71/release-set.yaml` under stable
...
0.71 is pre-stable. Before publication, release automation downloads the
...
control does not alter the NKF 0.71 format.
...
but NKF 0.71 does not define that policy. A consumer MUST NOT hide missing
...
The following remain deliberately unresolved in NKF 0.71:
...
profile decision is part of NKF 0.71 core.

## Minimal Example

A logical NKF 0.71 manifest:
...
| --- | --- | --- |
| NKF attempts to model every kind of knowledge immediately | The version becomes unusable and untestable | Keep 0.71 limited to evidence-backed Product and Technology profiles and add later roots from real needs |
