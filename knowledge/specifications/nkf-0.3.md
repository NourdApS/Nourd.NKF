---
id: nkf-0.3-specification
title: NKF 0.3 — Product And Technology Knowledge Format
type: specification
...
# NKF 0.3 — Product And Technology Knowledge Format

- **Predecessor canonical baseline:** `knowledge/specifications/nkf-0.2.md`
- **Predecessor canonical digest:** `336876774b4f2ee02a0a74b29616bdaf35df412edd8046ae3cd1cb18fc1b2a02`
- **Predecessor executable baseline:** `contracts/nkf/0.2/nkf.yaml`
- **Predecessor executable digest:** `df4e62f9f8e98bee2018cecdc205f92cdc191a8788e6a0ee8e2bb2378b86be07`
- **Canonical destination:** `knowledge/specifications/nkf-0.3.md`
- **Executable companion destination:** `contracts/nkf/0.3/nkf.yaml`
- **Governing successor direction:** [ADR 0109](../decisions/0109-publication-freeze-and-proven-self-adoption.md)
- **Independent governing inputs:** accepted NKF 0.2 authority lineage through
  [ADR 0104](../decisions/0104-accept-the-cancelled-state-pair.md), extended
...
format for durable governed knowledge. NKF 0.3 supports Product and Technology
...
NKF 0.3 is a pre-stable format version produced under the versioned contract
evolution accepted by [ADR 0076](../decisions/0076-versioned-contract-evolution.md).
The complete accepted NKF 0.2 authority lineage remains the predecessor
...
allocates the successor correction to NKF 0.3: publication freezes the exact
complete set, a published release is never replaced in place, and exact
...
declares `nkf_version` `0.3`; NKF 0.1 and NKF 0.2 remain immutable authority
...
NKF 0.3 defines Common contracts and two concrete Root Profiles: Product and
...
identities. They are all governed by the one NKF version coordinate, `0.3`.
Profile identity is not an independent version coordinate.

The sole NKF 0.3 record definition includes explicit responsibility bindings.
...
migrated by their consumers, but MUST NOT be reported as native NKF 0.3
...
version `0.3` alone does not identify editorial changes to a draft.

NKF 0.3 uses OKF 0.2 as its interoperability baseline. A later OKF release
...
contracts. NKF 0.3 does not silently rename or reinterpret them. A controlled
...
that directly contains `.nourd/`. Native NKF 0.3 fixes:
...
| --- | --- |
| `nkf_version` | Constant NKF version `"0.3"` |
...
top-level fields fail closed. Native NKF 0.3 has no `product_record`,
...
Every Product and Technology bundle using this successor NKF 0.3 revision
...
Native NKF 0.3 interprets the Markdown source body using CommonMark 0.31.2.
...
above, NKF 0.3 does not impose a narrower lexical format on the optional
...
Native NKF 0.3 defines no presentation-guidance field. Markdown is the default
...
Cross-bundle identity and typed relationships are deferred in NKF 0.3.
...
NKF 0.3 defines these record relationship types:
...
states the claimed original acceptance date. Native NKF 0.3 has no universal
...
last-modified times. NKF 0.3 does not define `stale_after` or a universal
freshness policy.
...
`nkf_version: "0.3"` is the only version coordinate and artifact digests bind
exact revisions.
...
2. executable YAML with identity `nkf.extension`, `nkf_version: "0.3"`, the
...
Native NKF 0.3 accepts no concrete extension. Portable presentation guidance
...
The fixed `.nourd` layout in the bundle contract is native NKF 0.3, not a
...
NKF 0.3 is based on the official
...
```text
contracts/nkf/0.3/schemas/
  bundle.schema.json   # urn:nkf:0.3:schema:bundle
  record.schema.json   # urn:nkf:0.3:schema:record
  validation-result.schema.json
                       # urn:nkf:0.3:schema:validation-result
```

The `0.3` component is the one NKF version coordinate. Each schema carries
...
contracts/nkf/0.3/schemas/release-manifest.schema.json
                       # urn:nkf:0.3:schema:release-manifest
```
...
closed UTF-8 JSON `nkf.validation-result` object with `nkf_version: "0.3"`.
...
`nkf.validation-snapshot` JSON with `nkf_version: "0.3"` and one entry per
...
snapshot, its core artifact bindings match the current accepted NKF 0.3
...
process. NKF 0.3 does not infer a hidden acceptance change, poll an authority,
...
NKF 0.3 uses one content-addressed archive attached to a Github Release in
...
They are not NKF versions. `nkf_version: "0.3"` remains the only format
version.

### Release Manifest

The archive contains exactly one UTF-8 JSON `release-manifest.json` and one
UTF-8 YAML `contracts/nkf/0.3/release-set.yaml`. The release manifest has
contract identity `nkf.release-manifest` and `nkf_version: "0.3"`; the release
...
- `knowledge/specifications/nkf-0.3.md`; and
- `contracts/nkf/0.3/nkf.yaml`.

`schemas` contains exactly four entries in exact identity order:

1. `urn:nkf:0.3:schema:bundle`;
2. `urn:nkf:0.3:schema:record`;
3. `urn:nkf:0.3:schema:release-manifest`; and
4. `urn:nkf:0.3:schema:validation-result`.
...
members and modes declared by `contracts/nkf/0.3/release-set.yaml` under stable
...
0.3 is pre-stable. Before publication, release automation downloads the
...
control does not alter the NKF 0.3 format.
...
but NKF 0.3 does not define that policy. A consumer MUST NOT hide missing
...
The following remain deliberately unresolved in NKF 0.3:
...
profile decision is part of NKF 0.3 core.

## Minimal Example

A logical NKF 0.3 manifest:
...
| --- | --- | --- |
| NKF attempts to model every kind of knowledge immediately | The first version becomes unusable and untestable | Keep 0.3 limited to evidence-backed Product and Technology profiles and add later roots from real needs |
