...
title: NKF 0.5 — Product And Technology Knowledge Format
type: specification
summary: NKF 0.5 adds stable document nodes, declared lifecycle state, a deterministic knowledge graph, contextual freshness, conservative impact review, and revision-bound whole-root readiness to the Product and Technology contracts.
created_at: 2026-08-13T10:00:00Z
record_lifecycle: living
record_status: draft
task: NKF-026
decision_authority: Codex technical reviewer under the Human Product Owner's explicit delegation for exact technical derivation within the adopted NKF-025 direction
---

# NKF 0.5 — Product And Technology Knowledge Format

- **Predecessor canonical baseline:** `knowledge/specifications/nkf-0.4.md`
- **Predecessor canonical digest:** `7298d1a55dcd74d4cc96368648aadbd6a70b5cf4c62d2a1c7f528e7c9181bab1`
- **Predecessor executable baseline:** `contracts/nkf/0.4/nkf.yaml`
- **Predecessor executable digest:** `a84fcc1e99567b6716e3281efedbbc87c978ffa465cd4cb5d8cac1d46ad0d217`
- **Canonical destination:** `knowledge/specifications/nkf-0.5.md`
- **Executable companion destination:** `contracts/nkf/0.5/nkf.yaml`
- **Governing successor direction:** [ADR 0115](../decisions/0115-adopt-freshness-and-deterministic-knowledge-graph-direction.md)
- **Independent governing inputs:** the accepted NKF 0.4 authority pair through
  [ADR 0113](../decisions/0113-accept-the-nkf-0-4-authority-pair.md), the exact
  adopted Design bound by [ADR 0115](../decisions/0115-adopt-freshness-and-deterministic-knowledge-graph-direction.md), and immutable publication and
  self-adoption boundaries from
...
> This exact revision is the canonical NKF 0.5 specification when accepted
...
> It realizes only the Product direction adopted by
> [ADR 0115](../decisions/0115-adopt-freshness-and-deterministic-knowledge-graph-direction.md),
> governs repositories that deliberately declare NKF 0.5, and does not alter
> the immutable authority of NKF 0.1, NKF 0.2, NKF 0.3, or NKF 0.4.

## Purpose

The **Nourd Knowledge Format (NKF)** is a human-readable, machine-verifiable
format for durable governed knowledge. NKF 0.5 supports Product and Technology
...
NKF 0.5 is a pre-stable format version produced under the versioned contract
evolution accepted by [ADR 0076](../decisions/0076-versioned-contract-evolution.md).
The complete accepted NKF 0.4 authority pair and lineage remain the predecessor
meaning, including the Product and Technology profiles, Decision Applicability
Gate, deep-link and title rules, four-state Task vocabulary, immutable release
set, and public Adopt workflow. [ADR 0115](../decisions/0115-adopt-freshness-and-deterministic-knowledge-graph-direction.md) adopts the Product direction for
stable document nodes, declared lifecycle, a deterministic declared graph,
contextual freshness, conservative impact review, and revision-bound
whole-root readiness. Publication still freezes the exact complete set, a
published release is never replaced in place, exact-candidate Adopt and audit
precede publication, and ordinary public producer self-adoption and audit
precede merge. This revision governs a repository only after it is accepted
and that repository deliberately declares `nkf_version` `0.5`; earlier NKF
versions remain immutable authority for repositories that declare them.
...
declares NKF 0.4, as required by the two-stage publication sequence. Its
frontmatter is therefore an exact 0.4-governed proposal bootstrap envelope.
If a later Decision accepts these exact bytes, candidate-bound 0.5 Adopt
preserves them byte for byte and records an `accepted_bootstrap_lock` in the
0.5 declaration. That lock binds the exact source digest, the exact accepting
Decision ID and digest, predecessor state `living`/`draft` plus the exact
serialized decision authority and Task, and current
declaration state `immutable`/`accepted`. The accepting Decision is the sole
authorized transition basis; the predecessor fields then become inert
historical bytes while current lifecycle, authority, and Task binding come
only from the 0.5 declaration. Every other migrated source uses
`legacy-locked`, whose separate predecessor and initial-declaration
projections are linked by the exact conversion rules below.
This closed bootstrap exception exists only for the version's canonical
Specification and cannot accept itself, infer acceptance from validation, or
permit new 0.5-native sources to persist mutable state in Markdown.
...
This NKF 0.5 revision preserves that complete predecessor meaning and adds
only the exact direction adopted by [ADR 0115](../decisions/0115-adopt-freshness-and-deterministic-knowledge-graph-direction.md): stable document nodes, stable
source paths across lifecycle changes, one declared graph, contextual
freshness, conservative impact closure, baseline confirmation, and
revision-bound receipts and readiness. Earlier versions remain immutable
authority for repositories that declare them, and consumers migrate
deliberately.

## Scope

NKF 0.5 defines Common contracts and two concrete Root Profiles: Product and
...
NKF 0.3, or NKF 0.4 to NKF 0.5 is `breaking` and requires repository-owner
approval before mutation, even though migration preserves canonical Markdown
bytes and historical authority. The break allocates stable lifecycle paths,
document-node identities, a declared graph, and a reviewed graph baseline.
An exact NKF 0.5-to-0.5 refresh is `non-breaking`, requires no migration, and
...
`nkf_version`. NKF 0.5 uses the unversioned canonical identities `nkf.bundle`,
...
identities. They are all governed by the one NKF version coordinate, `0.5`.
Profile identity is not an independent version coordinate.

The sole NKF 0.5 record definition includes explicit responsibility bindings.
...
migrated by their consumers, but MUST NOT be reported as native NKF 0.5
...
version `0.5` alone does not identify editorial changes to a draft.

NKF 0.5 uses OKF 0.2 as its interoperability baseline. A later OKF release
...
contracts. NKF 0.5 does not silently rename or reinterpret them. A controlled
...
that directly contains `.nourd/`. Native NKF 0.5 fixes:
...
| --- | --- |
| `nkf_version` | Constant NKF version `"0.5"` |
...
top-level fields fail closed. Native NKF 0.5 has no `product_record`,
...
becomes its document-node ID during migration. A new 0.5 Task is created with
...
NKF 0.5, outside canonical Markdown, and governed by the baseline contract
...
native 0.5 checker matches a supplied external observation by that exact ID
...
Every Product and Technology bundle using this successor NKF 0.5 revision
...
Every Task and Design source path that exists at 0.5 migration is frozen as
...
Migration from any supported predecessor preserves every canonical Markdown
byte and every existing Task and Design source path. It upgrades declarations,
...
rewritten 0.5 sources use the native 0.5 envelope.
...
Native 0.5 frontmatter is deliberately stable orientation. Mutable lifecycle,
...
a winner or rewrite either representation. Native 0.5 frontmatter MUST NOT
...
migration fails before mutation. The mapping is required exactly when a
...
the exemption and requires the native 0.5 envelope. This narrow rule preserves
accepted immutable records without leaving two current lifecycle authorities.

The version-canonical NKF 0.5 Specification MAY instead carry the single
`accepted_bootstrap_lock` defined by Source Drafting Provenance. The lock is a
closed declaration that binds predecessor version `0.4`, the exact source
digest, the exact accepted Decision ID and source digest, predecessor state
`living`/`draft` plus the exact serialized decision authority and Task, and
current state `immutable`/`accepted`. Its accepting
Decision MUST explicitly accept this exact Specification source and executable
companion. Only that Decision supplies acceptance; the lock and validation do
not. The exception is forbidden for every other record, and any later source
byte change requires a successor governed revision rather than editing the
locked record.
...
state key in native 0.5 source emits
...
Native NKF 0.5 interprets the Markdown source body using CommonMark 0.31.2.
...
above, NKF 0.5 does not impose a narrower lexical format on the optional
...
Native NKF 0.5 defines no presentation-guidance field. Markdown is the default
...
Cross-bundle identity and typed relationships are deferred in NKF 0.5.
...
NKF 0.5 defines these record relationship types:
...
```json
{"contract":"nkf.graph-revision","nkf_version":"0.5","bundle":"<bundle-id>","profile":"<profile-id>","nodes":[{"node":<node-reference>,"revision":"<hex>"}],"edges":[<normalized-authored-edge>],"external_dependencies":[<normalized-external-dependency>],"authority_inputs":[<normalized-authority-input>],"policy":{"id":"nkf.freshness-policy.0.5","sha256":"<hex>"}}
```
...
NKF 0.5 distributes exactly one immutable policy named
`nkf.freshness-policy.0.5` at
`contracts/nkf/0.5/freshness-policy.yaml`. Its release manifest binds the exact
...
0.5. Every supported non-context, non-historical relationship consequence is
...
`0.5`. It binds bundle identity, selected profile, graph revision, policy
...
Freshness receipts use contract `nkf.freshness-receipt`, version `0.5`, and are
...
one exact supplied observation identity. NKF 0.5 supports no free-form
...
states the claimed original acceptance date. Native NKF 0.5 has no universal
...
through an explicit 0.5 freshness declaration, policy, purpose, and bound
...
`nkf_version: "0.5"` is the only version coordinate and artifact digests bind
exact revisions.
...
2. executable YAML with identity `nkf.extension`, `nkf_version: "0.5"`, the
...
Native NKF 0.5 accepts no concrete extension. Portable presentation guidance
...
The fixed `.nourd` layout in the bundle contract is native NKF 0.5, not a
...
NKF 0.5 is based on the official
...
```text
contracts/nkf/0.5/schemas/
  bundle.schema.json   # urn:nkf:0.5:schema:bundle
  record.schema.json   # urn:nkf:0.5:schema:record
  validation-result.schema.json
                       # urn:nkf:0.5:schema:validation-result
...
The `0.5` component is the one NKF version coordinate. Each schema carries
...
contracts/nkf/0.5/schemas/release-manifest.schema.json
                       # urn:nkf:0.5:schema:release-manifest
```
...
closed UTF-8 JSON `nkf.validation-result` object with `nkf_version: "0.5"`.
...
`nkf.validation-snapshot` JSON with `nkf_version: "0.5"` and one entry per
...
`knowledge_graph` contains the exact policy binding, candidate graph revision,
nullable baseline graph revision, baseline state `confirmed`, `missing`,
...
snapshot, its core artifact bindings match the current accepted NKF 0.5
...
process. NKF 0.5 does not infer a hidden acceptance change, poll an authority,
...
NKF 0.5 uses one content-addressed archive attached to a Github Release in
...
They are not NKF versions. `nkf_version: "0.5"` remains the only format
version.

### Release Manifest

The archive contains exactly one UTF-8 JSON `release-manifest.json` and one
UTF-8 YAML `contracts/nkf/0.5/release-set.yaml`. The release manifest has
contract identity `nkf.release-manifest` and `nkf_version: "0.5"`; the release
set has contract identity `nkf.release-set` and the same version.

The manifest is one closed JSON object with exactly eight required top-level
fields in this logical shape:

```text
contract, nkf_version, source, checker, authority, freshness_policy, schemas, files
```
...
- `knowledge/specifications/nkf-0.5.md`; and
- `contracts/nkf/0.5/nkf.yaml`.

`freshness_policy` binds identity `nkf.freshness-policy.0.5`, exact path
`contracts/nkf/0.5/freshness-policy.yaml`, and its SHA-256 digest.

`schemas` contains exactly seven entries in exact identity order:

1. `urn:nkf:0.5:schema:bundle`;
2. `urn:nkf:0.5:schema:record`;
3. `urn:nkf:0.5:schema:graph-baseline`;
4. `urn:nkf:0.5:schema:freshness-receipt`;
5. `urn:nkf:0.5:schema:freshness-policy`;
6. `urn:nkf:0.5:schema:release-manifest`; and
7. `urn:nkf:0.5:schema:validation-result`.
...
The manifest's checker, authority, freshness-policy, and schemas bindings repeat identities for
...
`contracts/nkf/0.5/release-set.yaml` is the sole exact coverage, membership,
class, and mode enumeration for the NKF 0.5 complete set. It is a closed YAML
...
executable companion, evaluation policy, release-set contract, release
manifest, derived Schemas, checker, adopter, authoring protocol, onboarding
protocol, release protocol, adoption protocol, portable skills, registered
host-adapter instruction content, valid Product fixtures, valid Technology
fixtures, public Product examples, public Technology examples, and public
...
members and modes declared by `contracts/nkf/0.5/release-set.yaml` under stable
...
0.5 is pre-stable. Before publication, release automation downloads the
...
control does not alter the NKF 0.5 format.
...
but NKF 0.5 does not define that policy. A consumer MUST NOT hide missing
...
The following remain deliberately unresolved in NKF 0.5:
...
profile decision is part of NKF 0.5 core.

## Minimal Example

A logical NKF 0.5 manifest:
...
| --- | --- | --- |
| NKF attempts to model every kind of knowledge immediately | The version becomes unusable and untestable | Keep 0.5 limited to evidence-backed Product and Technology profiles and add later roots from real needs |
