...
created_at: 2026-08-13T19:56:20Z
---
...
- **Canonical destination:** `knowledge/specifications/nkf-0.6.md`
- **Executable companion destination:** `contracts/nkf/0.6/nkf.yaml`
...
> It realizes only the predecessor meaning and the direction adopted by
> [ADR 0121](../decisions/0121-adopt-the-nkf-0-6-corrective-and-licensing-direction.md),
...
promotion input at
`knowledge/evidence/release/nkf-0.6-producer-promotion.yaml`. That input is a
...
`.nourd/knowledge/records/nkf-0.6-specification.yaml`, including the prospective
...
document representation, and creates the native 0.6 Specification declaration
at the exact supplied declaration destination, lifecycle `immutable`, status
`accepted`, bound to the exact accepting
Decision, source digest, Task `NKF-027`, and the complete exact section and
...
Adopt observes the exact native 0.6 declaration and returns `current` without
running promotion again. Any non-producer repository, missing or extra
...
state `living`/`draft`, Task `NKF-026`, and current state
...
- `knowledge/specifications/nkf-0.6.md`; and
- `contracts/nkf/0.6/nkf.yaml`.
