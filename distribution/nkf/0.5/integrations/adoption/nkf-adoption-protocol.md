...
| Native unpinned NKF 0.5 repository | None beyond release access | `updated` |
| Pinned NKF 0.5 repository on another supported release | None | `updated` |
| Exact recommended 0.5 pin and integration | None | `current` |

Compatibility is relative to the declared predecessor. Every 0.1-through-0.4
path to 0.5 is breaking and requires explicit repository-owner approval before
mutation; 0.5-to-0.5 refresh is non-breaking. Tooling enforces these accepted
...
plan. Migration constructs the exact 0.5 graph outside the repository; a named
