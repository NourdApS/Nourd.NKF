...
| Native unpinned NKF 0.4 repository | None beyond release access | `updated` |
| Pinned NKF 0.4 repository on another supported release | None | `updated` |
| Exact recommended 0.4 pin and integration | None | `current` |

Compatibility is relative to the declared predecessor. The 0.1-to-0.4 and
0.2-to-0.4 paths are breaking and require explicit approval before mutation;
0.4-to-0.4 refresh is non-breaking. Tooling enforces these accepted
...
and integration feasibility. It then stages one complete transaction and
...
knowledge; it does not accept canonical meaning.
