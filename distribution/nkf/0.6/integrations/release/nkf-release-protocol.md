...
0.6 release. It is operational process carried in the complete versioned set;
...
`contracts/nkf/0.6/release-set.yaml`, exact `LICENSE`, `NOTICE`, and
build-input-derived `THIRD_PARTY_NOTICES.md` artifacts, and a passing
`npm run nkf:check`. The
...
5. Verify the accepted Specification, executable companion, all derived
   Schema digests, the exact Apache-2.0 license and NOTICE, and complete
   checker/adopter build-input notices.
...
- use the producer-only accepted 0.5-to-0.6 promotion input and accepting
  Decision without repository-owner breaking approval;
...
- promote the exact candidate Specification from Evidence to its native 0.6
  declaration, migrate the bundle to 0.6, and pass the extracted candidate
  checker;
...
accepted authority, Schema, and licensing bindings, checker and adopter
execution, 0.1 through 0.4 migrations, non-breaking 0.5 update, same-version
refresh, host-superset preservation,
